...
class WalletMigrationTest(BitcoinTestFramework):
    ...
    def migrate_wallet(self, old_version, wallet_type, extra_args_old=[]):
        """Common migration test logic"""
        self.log.info(f"Testing {wallet_type} wallet migration from {old_version}")
        ...
        self.add_nodes(
            2,
            versions=[
                old_version,  # Old version node
                None,        # Current version for testing migration
            ],
        )
        
        # Start the old node with specified args
        self.log.info(f"Starting old node ({old_version}) with {wallet_type} wallet...")
        ...
        if "non-HD" in wallet_type:
            assert 'hdmasterkeyid' not in old_wallet_info, "Expected non-HD wallet"
        else:
            assert 'hdmasterkeyid' in old_wallet_info, f"Expected HD wallet for {wallet_type}"
        ...
        for addr in old_addresses:
            ...
            assert addr_info['ismine'], f"Address {addr} should still be mine after migration"
        
        # Verify change addresses if any
        if change_addresses:
            for change_addr in change_addresses:
                addr_info = self.nodes[1].getaddressinfo(change_addr)
                assert addr_info['ismine'], f"Change address {change_addr} should still be mine"
        
        self.log.info(f"✓ All addresses verified as owned after migration")
        ...
        self.stop_node(1)
        self.log.info(f"✓ {wallet_type} wallet migration from {old_version} completed successfully!\n")
