...
class SilentLabelTest(BitcoinTestFramework):
    ...
    def test_address_label(self):
        ...
        assert_raises_rpc_error(-4, "Error: Use `getspaddress` RPC to generate silent payment address",
                sp_wallet_01.getnewaddress, address_type='silent-payment')

        self.log.info("Confirm that calling getspaddress without the label parameter will generate same address.")
        assert_equal(sp_wallet_01.getspaddress(), sp_wallet_01.getspaddress())

        self.log.info("Confirm that calling getspaddress with different labels will generate different address.")
        label01 = self.random_string(8)
        label02 = self.random_string(8)

        assert_equal(sp_wallet_01.getspaddress(label01), sp_wallet_01.getspaddress(label01))
        assert(sp_wallet_01.getspaddress(label01) != sp_wallet_01.getspaddress(label02))
        ...
        for _ in range(97):
            ...
            assert_equal(identifier, int(sp_wallet_01.getspaddress(label)[4:6]))
            identifier = identifier + 1

        label = self.random_string(8)
        assert_raises_rpc_error(-12, "No addresses available", sp_wallet_01.getspaddress, label=label)
