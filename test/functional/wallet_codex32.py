...
"""Test wallet codex32 seed backup and recovery."""
...
class WalletCodex32Test(BitcoinTestFramework):
    ...
    def recover_encrypted_wallet(self):
        ...
        with WalletUnlock(enc_wallet_w1, "123456"):
            codex32_seed = enc_wallet_w1.exposesecret()[0]
        self.log.info(codex32_seed)

        self.nodes[1].recoverwalletfromseed(wallet_name="enc_w1_recover", codex32_seed=codex32_seed)

        enc_wallet_w1_recover = self.nodes[1].get_wallet_rpc("enc_w1_recover")
        assert "unlocked_until" not in enc_wallet_w1_recover.getwalletinfo()
