...
class WalletPrivateBroadcastTest(BitcoinTestFramework):
    ...
    def setup_nodes(self):
        ...
        self.extra_args = [
            [
                ...
                "-v2transport=0",
                "-test=addrman",
                # Enable debug logs to validate private broadcast behavior
            ...
            [
                ...
                "-v2transport=0",
                # Enable debug logs to observe transaction receipt
                "-debug=mempool",
            ],
        ]
        super().setup_nodes()
    ...
    def check_log_message(self, node, message):
        """Check if a message appears in the node's debug.log file."""
    ...
    def run_test(self):
        ...
        # Verify the private broadcast log message appears (sender sent INV via private broadcast)
        priv_broadcast_msg = f"P2P handshake completed, sending INV for txid={txid1}"
        self.wait_until(lambda: self.check_log_message(sender, priv_broadcast_msg), timeout=30)
        self.log.info("PASS: Private broadcast log message found (INV sent via SOCKS5)")
        ...
        # Verify receiver accepted the transaction to mempool (log validation)
        receiver_accepted_msg = f"accepted {txid1}"
        self.wait_until(lambda: self.check_log_message(receiver, receiver_accepted_msg), timeout=30)
        self.log.info("PASS: Receiver accepted transaction to mempool (log validated)")
        ...
        # Verify the private broadcast log message for the rebroadcast (txid1)
        priv_broadcast_resubmit_msg = f"P2P handshake completed, sending INV for txid={txid1}"
        self.wait_until(lambda: self.check_log_message(sender, priv_broadcast_resubmit_msg), timeout=30)
        self.log.info("PASS: Private broadcast log validated for resubmitted transaction")
