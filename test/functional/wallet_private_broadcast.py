...
- test_public_tx_skipped_when_privatebroadcast_enabled: Public txs not rebroadcast privately
...
class WalletPrivateBroadcastTest(BitcoinTestFramework):
    ...
    def test_public_tx_skipped_when_privatebroadcast_enabled(self):
        """Test that publicly-sent txs are not rebroadcast when -privatebroadcast enabled."""
        self.log.info("Testing that public txs are skipped when -privatebroadcast enabled...")
        ...
        # - txid_public (no flag): SKIPPED to prevent privacy leak
        ...
        skip_public_msg = f"skipping tx {self.txid_public} originally sent via public broadcast"

        with sender.assert_debug_log(expected_msgs=[skip_public_msg], unexpected_msgs=[skip_private_msg]):
            self.trigger_wallet_resubmit(sender)

        self.log.info("Public transaction correctly skipped")

        # Verify txid_private triggered new private broadcast
        ...
        self.log.info("Private transaction rebroadcast via private broadcast")
    ...
    def run_test(self):
        ...
        self.test_public_tx_skipped_when_privatebroadcast_enabled()
