...
class ConfArgsTest(BitcoinTestFramework):
    ...
    def test_empty_conn_args(self):
        self.log.info('Test empty -addnode, -connect and -seednode values are ignored')
        node = self.nodes[0]
        util.append_config(node.datadir_path, ['addnode=', 'connect=', 'seednode='])

        with node.assert_debug_log(
                expected_msgs=[
                    'Ignoring empty -addnode value',
                    'Ignoring empty -connect value',
                    'Ignoring empty -seednode value',
                ],
                unexpected_msgs=[
                    # -connect being set at all keeps disabling automatic
                    # connections, even though its empty value is ignored as a
                    # connection target.
            ...
            # Stop the node inside the assertion, so that the whole life span of
            # the node is covered by the unexpected messages check above.
            self.stop_node(0)
    ...
    def test_seed_peers(self):
        ...
        # Empty -addnode/-seednode values are ignored, so they must not delay
        # the fallback either.
        assert not peer_dat.exists()
        with self.nodes[0].assert_debug_log(expected_msgs=[
                "Loaded 0 addresses from peers.dat",
            ...
            self.start_node(0, extra_args=['-dnsseed=0', '-fixedseeds=1', '-addnode=', '-seednode='])
        self.stop_node(0)
