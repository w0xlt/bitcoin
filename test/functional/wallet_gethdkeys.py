...
class WalletGetHDKeyTest(BitcoinTestFramework):
    # BIP-93 reference HRP "ms". Strings regenerated via Codex32SecretEncode.
    CODEX32_SECRET = "ms10f2tvsugydzy9ggegddt5tyamkawpve4ukxvvdntmhwvr2f9se3sf6r54sr3edm5vrhz3t2"
    CONFLICTING_CODEX32_SECRET = "ms10testsugydzy9ggegddt5tyamkawpve4ukxvvdntmhwvr2f9se3sf6r54sdpfzqk7th0jcn"
    SHORT_CODEX32_SECRET = "ms10f2tvsqqqsyqcyq5rqwzqfpg9scrgwzsvqrkcka4a5l"
    ...
    def run_test(self):
        ...
        self.test_addhdkey_multisig_descriptor()
    ...
    def test_basic_gethdkeys(self):
        ...
        # New descriptor wallets receive a codex32 backup of their auto-generated HD root.
        assert_equal(xpub_info[0]["has_codex32"], True)
    ...
    def test_ranged_imports(self):
        ...
        for x in xpub_info:
            if x["xpub"] == active_xpub:
                assert_equal(x["has_codex32"], True)
        ...
        for rpc_req_resp in rpcs_req_resp:
            ...
            for hdkeys_info in hdkeys_response:
                if hdkeys_info["xpub"] == within_wallet_xpub:
                    ...
                    assert_equal(hdkeys_info["has_codex32"], True)
    ...
    def test_addhdkey(self):
        ...
        # Validation errors. The share-rejection branch in addhdkey requires a
        # valid BIP-93 share string, which our codec does not generate; that
        # branch is exercised by the codex32 unit-tests' decode-side coverage.
        ...
        assert "codex32_secrets" in wallet.getwalletinfo()["flags"]
        ...
        assert_raises_rpc_error(-5, f"HD key {blank_xpub} is already known", wallet.addhdkey, self.CODEX32_SECRET.upper())
    ...
    def test_addhdkey_existing_descriptor(self):
        ...
        assert_equal(wallet.gethdkeys(active_only=True), [])
        ...
        active_hdkeys_info = wallet.gethdkeys(active_only=True)
        assert_equal(len(active_hdkeys_info), 1)
        assert_equal(active_hdkeys_info[0]["xpub"], source_info["xpub"])
        assert_greater_than(len(active_hdkeys_info[0]["descriptors"]), 1)
        ...
        self.nodes[0].createwallet("addhdkey_existing_default_descriptors", blank=True)
        default_wallet = self.nodes[0].get_wallet_rpc("addhdkey_existing_default_descriptors")
        default_descs = source_wallet.listdescriptors(True)["descriptors"]
        for desc in default_descs:
            desc["active"] = False
        assert_equal([res["success"] for res in default_wallet.importdescriptors(default_descs)], [True] * len(default_descs))
        assert_equal(default_wallet.gethdkeys(active_only=True), [])

        assert_equal(default_wallet.addhdkey(source_info["codex32"])["xpub"], source_info["xpub"])
        active_hdkeys_info = default_wallet.gethdkeys(active_only=True)
        assert_equal(len(active_hdkeys_info), 1)
        assert_equal(active_hdkeys_info[0]["xpub"], source_info["xpub"])
        assert_equal(len(active_hdkeys_info[0]["descriptors"]), len(default_descs))
        default_wallet.getnewaddress()
    ...
    def test_addhdkey_multisig_descriptor(self):
        self.log.info("addhdkey stores codex32 secrets by root in multisig descriptors")
        source_wallet = self.ensure_wallet_loaded("addhdkey_blank")
        source_info = source_wallet.gethdkeys(active_only=True, private=True, codex32=True)[0]

        def_wallet = self.nodes[0].get_wallet_rpc(self.default_wallet_name)
        outside_xpub = def_wallet.gethdkeys()[0]["xpub"]

        self.nodes[0].createwallet("addhdkey_multisig_descriptor", blank=True)
        wallet = self.nodes[0].get_wallet_rpc("addhdkey_multisig_descriptor")
        desc = descsum_create(f"wsh(multi(2,{source_info['xprv']}/*,{outside_xpub}/*))")
        assert_equal(wallet.importdescriptors([{"desc": desc, "timestamp": "now"}])[0]["success"], True)

        assert_equal(wallet.addhdkey(source_info["codex32"])["xpub"], source_info["xpub"])

        hdkeys_by_xpub = {info["xpub"]: info for info in wallet.gethdkeys(private=True, codex32=True)}
        assert_equal(hdkeys_by_xpub[source_info["xpub"]]["has_codex32"], True)
        assert_equal(hdkeys_by_xpub[source_info["xpub"]]["codex32"], source_info["codex32"])
        assert_equal(hdkeys_by_xpub[outside_xpub]["has_codex32"], False)
        assert "codex32" not in hdkeys_by_xpub[outside_xpub]
