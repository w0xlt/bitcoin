...
class P2PStaleTipTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        self.test_blocks_mode_serves_tracked_stale_block()
    ...
    def test_blocks_mode_serves_tracked_stale_block(self):
        self.log.info("Test blocks mode announces and serves tracked stale block data")
        node = self.nodes[0]
        peer = self.connect_peer(feature_data=b"\x01")
        ...
        # The stale tip block was never connected, but its data is available
        # and the tip is tracked, so the announcement offers the block data
        # and a request for it succeeds.
        ...
        peer.send_without_ping(msg_getdata([CInv(MSG_BLOCK, block.hash_int)]))
        peer.wait_for_block(block.hash_int)
        self.nodes[0].disconnect_p2ps()
