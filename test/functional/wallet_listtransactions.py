...
class ListTransactionsTest(BitcoinTestFramework):
    ...
    def run_coinjoin_test(self):
        ...
        assert_equal(tx_info["fee_known"], False)
        ...
        assert_array_result(tx_info["details"], {"category": "mixed"}, {
        ...
        # The unfiltered history reports the mixed summary plus a receive entry
        # for the wallet-owned output, but no send entries.
        history = [entry for entry in self.nodes[0].listtransactions("*", 100) if entry["txid"] == txid_join]
        assert_equal(len(history), 2)
        assert_equal(any(entry["category"] == "send" for entry in history), False)
        assert_equal(sum(entry["amount"] for entry in history), expected_amount)
        assert_array_result(history, {"category": "mixed"}, {
            "amount": -input_0["amount"],
            "wallet_debit": input_0["amount"],
            "wallet_credit": expected_credit,
            "involves_mixed_inputs": True,
