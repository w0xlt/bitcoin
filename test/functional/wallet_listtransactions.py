...
class ListTransactionsTest(BitcoinTestFramework):
    ...
    def run_coinjoin_test(self):
        ...
        assert_equal(history[0]["category"], "mixed")
        assert_equal(history[0]["amount"], expected_amount)
        assert "fee" not in history[0]
        assert_equal(history[0]["wallet_debit"], input_0["amount"])
        assert_equal(history[0]["wallet_credit"], expected_credit)
