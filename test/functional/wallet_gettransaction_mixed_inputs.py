...
class WalletGetTransactionMixedInputsTest(BitcoinTestFramework):
    ...
    def assert_mixed_input_send_summary(self, entry, txid, total_wallet_input_value, wallet_credit, include_txid):
        if include_txid:
            assert_equal(entry["txid"], txid)
        # The summary amount is the total value of wallet-owned inputs with its
        # sign reversed. It has no address or vout because it is not tied to one
        # output, and no fee because the wallet's fee share is unknown.
        assert_equal(entry["category"], "send")
        assert_equal(entry["amount"], -total_wallet_input_value)
        assert "address" not in entry
        assert "vout" not in entry
        assert "fee" not in entry
        self.assert_mixed_fields(entry, total_wallet_input_value, wallet_credit)
    ...
    def assert_mixed_history_entries(self, entries, txid, expected_net, address, vout, wallet_debit, wallet_credit, include_txid=True):
        """Check that a conservative mixed-input transaction has one send summary
        reporting the total value of wallet-owned inputs with its sign reversed,
        plus a receive entry for the wallet-owned output, with entry amounts summing
        to the wallet net change."""
        ...
        self.assert_mixed_input_send_summary(send_entries[0], txid, wallet_debit, wallet_credit, include_txid=include_txid)
    ...
    def assert_wallet_view(self, wallet_name, wallet, txid, label, address, vout, wallet_debit, wallet_credit, before_blockhash):
        wallet_balance_change = wallet_credit - wallet_debit

        tx_info = wallet.gettransaction(txid)
        # No fee is separated when the wallet's fee share is unknown, so amount
        # equals the wallet balance change.
        assert "fee" not in tx_info
        assert_equal(tx_info["amount"], wallet_balance_change)
        self.assert_mixed_fields(tx_info, wallet_debit, wallet_credit)
        self.assert_mixed_history_entries(tx_info["details"], txid, wallet_balance_change, address, vout, wallet_debit, wallet_credit, include_txid=False)

        history = [entry for entry in wallet.listtransactions("*", 100) if entry["txid"] == txid]
        self.assert_mixed_history_entries(history, txid, wallet_balance_change, address, vout, wallet_debit, wallet_credit)
        ...
        self.assert_mixed_history_entries(since, txid, wallet_balance_change, address, vout, wallet_debit, wallet_credit)

        self.log.debug("%s mixed-input wallet balance change: %s", wallet_name, wallet_balance_change)
    ...
    def assert_wallet_funded_mixed_input_view(self, wallet, txid, external_address, wallet_debit, external_amount, burn_amount, fee):
        ...
        assert_equal(tx_info["amount"] + tx_info["fee"], wallet_credit - wallet_debit)
        self.assert_mixed_fields(tx_info, wallet_debit, wallet_credit, expect_fee=True)
        # Normal per-output accounting: every send entry has a vout and fee, so
        # no mixed-input send summary is needed.
    ...
    def test_zero_value_foreign_anchor_input(self, node, wallet, external_wallet):
        self.log.info("Test zero-value foreign anchor inputs keep normal per-output send and fee accounting")
    ...
    def test_zero_value_wallet_input(self, node, funder, alice, bob):
        ...
        # Alice does not store Bob's parent transaction, so she cannot retrieve the input value.
        ...
        # Alice's wallet-owned inputs have zero total value, so the mixed-input
        # send summary has a zero amount. The wallet-owned output is still
    ...
    def test_known_nonzero_foreign_input(self, node, funder, alice, bob):
        ...
        # input value), the wallet's fee share still cannot be determined, so
        # the transaction must use the conservative mixed-input send summary.
        alice_vout = find_vout_for_address(node, txid, alice_output)
        self.assert_wallet_view(
            wallet_name="alice",
            wallet=alice,
    ...
    def test_wallet_change_output(self, node, funder, alice, bob):
        ...
        # The conservative mixed-input send summary does not skip change: the change
        # output is reported as a receive entry alongside the send summary, so
