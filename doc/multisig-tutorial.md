...
Extract the xpub of each wallet at the BIP 87 multisig path using the `derivehdkey` RPC. This RPC derives an extended key at any BIP 32 path and returns the xpub along with origin information (fingerprint and path) in descriptor format.
...
for ((n=1;n<=3;n++))
do
  result=$(./build/bin/bitcoin-cli -signet -rpcwallet="participant_${n}" derivehdkey "m/87'/1'/0'")
...
Example output:
```
[xpub_1]=[1004658e/87'/1'/0']tpubDCBEcmVKbfC9KfdydyLbJ2gfNL88grZu1XcWSW9ytTM6fitvaRmVyr8Ddf7SjZ2ZfMx9RicjYAXhuh3fmLiVLPodPEqnQQURUfrBKiiVZc8/<0;1>/*
[xpub_2]=[a]tpubDC.../<0;1>/*
[xpub_3]=[b2f54e8c/87'/1'/0']tpubDE.../<0;1>/*
```
...
Use `derivehdkey` with `private: true` to get the xprv, then construct and import the descriptor:

```bash
for ((n=1;n<=3;n++))
do
  result=$(./build/bin/bitcoin-cli -signet -rpcwallet="participant_${n}" derivehdkey "m/87'/1'/0'" '{"private": true}')
  origin=$(echo $result | jq -r '.origin')
  ...
  priv_checksum=$(./build/bin/bitcoin-cli -signet getdescriptorinfo "$priv_desc" | jq -r '.checksum')
  import_desc="[{\"desc\": \"${priv_desc}#${priv_checksum}\", \"timestamp\": \"now\"}]"

  ./build/bin/bitcoin-cli -signet -rpcwallet="participant_${n}" importdescriptors "$import_desc"
done
```
