...
bool ChainstateManager::AcceptBlockHeader(const CBlockHeader& block, BlockValidationState& state, CBlockIndex** ppindex, bool min_pow_checked, bool& new_header)
{
    AssertLockHeld(cs_main);
    new_header = false;
...
bool ChainstateManager::ProcessNewBlockHeaders(std::span<const CBlockHeader> headers, bool min_pow_checked, BlockValidationState& state, const CBlockIndex** ppindex)
{
    ...
    bool accepted{true};
    {
        ...
        for (const CBlockHeader& header : headers) {
            ...
            accepted = AcceptBlockHeader(header, state, &pindex, min_pow_checked, new_header);
            CheckBlockIndex();
...
bool ChainstateManager::AcceptBlock(const std::shared_ptr<const CBlock>& pblock, BlockValidationState& state, CBlockIndex** ppindex, bool fRequested, const FlatFilePos* dbp, bool* fNewBlock, bool min_pow_checked)
{
    ...
    bool new_header{false};
    bool accepted_header{AcceptBlockHeader(block, state, &pindex, min_pow_checked, new_header)};
    CheckBlockIndex();

    if (!accepted_header)
        return false;

    if (new_header) MaybeNotifyAcceptedStaleTip(*pindex);
