...
{
    if (!txdata || !txdata->m_spent_outputs_ready) return false;
    if (m_txhash_cache) {
        return calculate_txhash(hash_out, field_selector, *m_txhash_cache, *txTo, txdata->m_spent_outputs, codeseparator_pos, nIn);
    }

    // Fallback for callers that do not thread a shared cache; correctness does
    // not depend on cache sharing.
    TxHashCache local_cache;
    return calculate_txhash(hash_out, field_selector, local_cache, *txTo, txdata->m_spent_outputs, codeseparator_pos, nIn);
}
