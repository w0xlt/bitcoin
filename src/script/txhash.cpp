...
static uint256 compute_annex_hash(const std::vector<CTxIn>& inputs, const std::vector<CTxOut>& prev_outputs, unsigned int idx) {
    if (prev_outputs[idx].scriptPubKey.IsPayToTaproot()) {
        const auto& stack = inputs[idx].scriptWitness.stack;
        if (stack.size() >= 2 && !stack.back().empty() && stack.back()[0] == ANNEX_TAG) {
            return sha256_bytes(stack.back());
        }
    }
    return SHA256_EMPTY;
}
...
static uint256 annex_hash(TxHashCache& cache, const std::vector<CTxIn>& inputs, const std::vector<CTxOut>& prev_outputs, unsigned int idx) EXCLUSIVE_LOCKS_REQUIRED(cache.mtx) {
    ...
    if (idx >= cache.hashed_annexes.size() || cache.hashed_annexes[idx].IsNull()) {
        ...
        cache.hashed_annexes[idx] = compute_annex_hash(inputs, prev_outputs, idx);
    }
...
bool calculate_txhash(
    ...
    if (!input_selected.empty()) {
        ...
        if ((inout_fields & TXFS_INPUTS_SCRIPTSIGS) != 0) {
            auto hash_item = [&](HashWriter& h, unsigned int i) { h << sha256_script(tx.vin[i].scriptSig); };
            if (all) {
        ...
        if ((inout_fields & TXFS_INPUTS_PREV_SCRIPTPUBKEYS) != 0) {
            auto hash_item = [&](HashWriter& h, unsigned int i) { h << sha256_script(prev_outputs[i].scriptPubKey); };
            if (all) {
        ...
        if ((inout_fields & TXFS_INPUTS_TAPROOT_ANNEXES) != 0) {
            auto hash_item = [&](HashWriter& h, unsigned int i) { h << compute_annex_hash(tx.vin, prev_outputs, i); };
            if (all) {
    ...
    if (!output_selected.empty()) {
        ...
        if ((inout_fields & TXFS_OUTPUTS_SCRIPTPUBKEYS) != 0) {
            auto hash_item = [&](HashWriter& h, unsigned int i) { h << sha256_script(tx.vout[i].scriptPubKey); };
            if (all) {
