...
public:
    ...
    SERIALIZE_METHODS(CBlockHeaderAndLengthShortTxIDs, obj) 
    {
        READWRITE(*(CBlockHeaderAndShortTxIDs*)obj);
        if (ser_action.ForRead()) {
            obj.txlens.clear();
            obj.txlens.reserve(obj.shorttxids.size());
            for (size_t i = 0; i < obj.shorttxids.size(); i++) {
                uint32_t len;
                READWRITE(VARINT(len));
                obj.txlens.emplace_back(len);
            }
        } else {
            for (size_t i = 0; i < obj.txlens.size(); i++)
                READWRITE(VARINT(obj.txlens[i]));
        }
    }
};
