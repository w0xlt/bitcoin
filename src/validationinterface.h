...
protected:
    ...
    /**
     * Whether this listener wants AcceptedStaleTip notifications. Header
    ...
    /**
     * Notifies listeners that a block header or block was accepted, is not on
     * the active chain, and has no more work than the active tip.
     *
     * Header notifications from one ProcessNewBlockHeaders() call are
     * coalesced to the last newly accepted header. A valid accepted prefix is
     * still notified if a later header in the batch is rejected. Already-known
     * headers do not generate another header notification. A later notification
     * for the same index may occur when its block data becomes available.
     *
     * Called on a background thread.
     */
