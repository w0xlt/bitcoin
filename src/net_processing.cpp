...
bool PeerManagerImpl::SendMessages(CNode& node)
{
...

    {
        ...
        if (auto tx_relay = peer.GetTxRelay(); tx_relay != nullptr) {
            ...
            std::optional<std::chrono::microseconds> next_inv_send_time;
            if (tx_relay->IsInvSendTimeReached(current_time)) {
                fSendTrickle = true;
                if (node.IsInboundConn()) {
                    next_inv_send_time = NextInvToInbounds(current_time, INBOUND_INVENTORY_BROADCAST_INTERVAL, node.m_network_key);
                } else {
                    next_inv_send_time = current_time + m_rng.rand_exp_duration(OUTBOUND_INVENTORY_BROADCAST_INTERVAL);
                }
            }

            auto tx_inventory_batch{tx_relay->StartTxInventoryBatch(fSendTrickle, next_inv_send_time)};
