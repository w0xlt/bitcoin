...
using node::MAX_CMPCTBLOCKS_INFLIGHT_PER_BLOCK;
...
node::BlockDownloadBlock MakeBlockDownloadBlock(const CBlockIndex& block)
...
/** Owned chain-work benchmark used by outbound peer chain-sync timeouts. */
struct ChainSyncBenchmark {
    node::BlockDownloadBlock m_block;
    CBlockLocator m_locator;
};

/**
 * Block-relay state with lifetime independent of validation's block index.
 * Access is mediated by PeerManagerImpl's block-relay snapshot helpers.
 */
struct BlockRelayState {
    ...
    struct ChainSyncTimeoutState {
        ...
        //! Owned block identity, work, and locator for the chain benchmark
        std::optional<ChainSyncBenchmark> m_work;
    ...
    //! Changes on every mutation of this peer's relay state
...
public:
    ...
    void BlockChecked(const std::shared_ptr<const CBlock>& block, const BlockValidationState& state) override
        EXCLUSIVE_LOCKS_REQUIRED(!m_peer_mutex, !m_block_relay_mutex);
    void BlockDataAvailable(const uint256& hash) override EXCLUSIVE_LOCKS_REQUIRED(cs_main);
    void NewPoWValidBlock(const CBlockIndex *pindex, const std::shared_ptr<const CBlock>& pblock) override
        EXCLUSIVE_LOCKS_REQUIRED(!m_most_recent_block_mutex, !m_block_relay_mutex);

    /** Implement NetEventsInterface */
    void InitializeNode(const CNode& node, ServiceFlags our_services) override EXCLUSIVE_LOCKS_REQUIRED(!m_peer_mutex, !m_tx_download_mutex, !m_block_relay_mutex);
    void FinalizeNode(const CNode& node) override EXCLUSIVE_LOCKS_REQUIRED(!m_peer_mutex, !m_headers_presync_mutex, !m_tx_download_mutex, !m_block_relay_mutex);
    bool HasAllDesirableServiceFlags(ServiceFlags services) const override;
    bool ProcessMessages(CNode& node, std::atomic<bool>& interrupt) override
        EXCLUSIVE_LOCKS_REQUIRED(!m_peer_mutex, !m_most_recent_block_mutex, !m_headers_presync_mutex, g_msgproc_mutex, !m_tx_download_mutex, !m_block_relay_mutex);
    bool SendMessages(CNode& node) override
        EXCLUSIVE_LOCKS_REQUIRED(!cs_main, !m_peer_mutex, !m_most_recent_block_mutex, g_msgproc_mutex, !m_tx_download_mutex, !m_block_relay_mutex);

    /** Implement PeerManager */
    void StartScheduledTasks(CScheduler& scheduler) override EXCLUSIVE_LOCKS_REQUIRED(!m_block_relay_mutex);
    void CheckForStaleTipAndEvictPeers() override EXCLUSIVE_LOCKS_REQUIRED(!m_block_relay_mutex);
    util::Expected<void, std::string> FetchBlock(NodeId peer_id, const CBlockIndex& block_index) override
        EXCLUSIVE_LOCKS_REQUIRED(!m_peer_mutex);
    bool GetNodeStateStats(NodeId nodeid, CNodeStateStats& stats) const override EXCLUSIVE_LOCKS_REQUIRED(!m_peer_mutex, !m_block_relay_mutex);
    ...
    void UpdateLastBlockAnnounceTime(NodeId node, int64_t time_in_seconds) override EXCLUSIVE_LOCKS_REQUIRED(!m_block_relay_mutex);
...
private:
    ...
    void ConsiderEviction(CNode& pto, Peer& peer, std::chrono::seconds time_in_seconds) EXCLUSIVE_LOCKS_REQUIRED(!cs_main, g_msgproc_mutex, !m_block_relay_mutex);

    /** If we have extra outbound peers, try to disconnect the one with the oldest block announcement */
    void EvictExtraOutboundPeers(NodeClock::time_point now) EXCLUSIVE_LOCKS_REQUIRED(cs_main, !m_block_relay_mutex);
    ...
    */
    void ProcessHeadersMessage(CNode& pfrom, Peer& peer,
        ...
        EXCLUSIVE_LOCKS_REQUIRED(!m_peer_mutex, !m_headers_presync_mutex, g_msgproc_mutex, !m_block_relay_mutex);
    ...
    void HeadersDirectFetchBlocks(CNode& pfrom, const Peer& peer, const CBlockIndex& last_header) EXCLUSIVE_LOCKS_REQUIRED(!m_block_relay_mutex);
    ...
    void UpdatePeerStateForReceivedHeaders(CNode& pfrom, Peer& peer, const CBlockIndex& last_header, bool received_new_header, bool may_have_more_headers)
        EXCLUSIVE_LOCKS_REQUIRED(g_msgproc_mutex, !m_block_relay_mutex);
    ...
    /** Internally synchronized owner of block request lifecycle state. */
    const std::unique_ptr<node::BlockDownloadManager> m_block_downloadman;
    ...
    /**
     * Protects the remaining per-peer and global block-relay state. Lock order
     * may be g_msgproc_mutex -> m_block_relay_mutex (and cs_main ->
     * m_block_relay_mutex), never the reverse. The mutex must be released
     * before manager/chain/mempool calls, peer or connman lookup, logging, or
     * network sends.
     */
    mutable Mutex m_block_relay_mutex;

    /** Map maintaining per-node block-relay state. */
    std::map<NodeId, BlockRelayState> m_node_states GUARDED_BY(m_block_relay_mutex);

    struct BlockRelayPeerSnapshot {
        bool m_requested_hb_cmpctblocks;
        bool m_provides_cmpctblocks;
        BlockRelayState::ChainSyncTimeoutState m_chain_sync;
        int64_t m_last_block_announcement;
        uint64_t m_generation;
    };

    struct BlockRelayGlobalSnapshot {
        int m_outbound_peers_with_protect_from_disconnect;
        std::list<NodeId> m_nodes_announcing_header_and_ids;
        uint64_t m_generation;
    };

    struct BlockRelaySnapshot {
        BlockRelayPeerSnapshot m_peer;
        BlockRelayGlobalSnapshot m_global;
    };

    struct BlockRelayPeerSnapshots {
        std::vector<std::pair<NodeId, BlockRelayPeerSnapshot>> m_peers;
        uint64_t m_global_generation;
    };

    struct RemovedBlockRelayState {
        bool m_empty;
    };

    void AddBlockRelayPeer(NodeId nodeid) EXCLUSIVE_LOCKS_REQUIRED(!m_block_relay_mutex);
    RemovedBlockRelayState RemoveBlockRelayPeer(NodeId nodeid) EXCLUSIVE_LOCKS_REQUIRED(!m_block_relay_mutex);
    std::optional<BlockRelayPeerSnapshot> GetBlockRelayPeerSnapshot(NodeId nodeid) const EXCLUSIVE_LOCKS_REQUIRED(!m_block_relay_mutex);
    std::optional<BlockRelaySnapshot> GetBlockRelaySnapshot(NodeId nodeid) const EXCLUSIVE_LOCKS_REQUIRED(!m_block_relay_mutex);
    std::optional<BlockRelayPeerSnapshots> GetBlockRelayPeerSnapshots(const std::vector<NodeId>& nodeids) const EXCLUSIVE_LOCKS_REQUIRED(!m_block_relay_mutex);
    bool SetCompactBlockState(NodeId nodeid, bool high_bandwidth) EXCLUSIVE_LOCKS_REQUIRED(!m_block_relay_mutex);
    bool SetLastBlockAnnouncement(NodeId nodeid, int64_t time_in_seconds) EXCLUSIVE_LOCKS_REQUIRED(!m_block_relay_mutex);
    bool TrySetChainSyncState(NodeId nodeid, uint64_t expected_generation,
                              BlockRelayState::ChainSyncTimeoutState state) EXCLUSIVE_LOCKS_REQUIRED(!m_block_relay_mutex);
    bool TryProtectPeer(NodeId nodeid, uint64_t expected_peer_generation,
                        uint64_t expected_global_generation) EXCLUSIVE_LOCKS_REQUIRED(!m_block_relay_mutex);
    bool TryUpdateNodesAnnouncingHeaderAndIDs(NodeId nodeid,
                                               uint64_t expected_peer_generation,
                                               uint64_t expected_global_generation,
                                               std::list<NodeId> nodes) EXCLUSIVE_LOCKS_REQUIRED(!m_block_relay_mutex);
    /** Linearize an external action against one owned relay snapshot. */
    bool TryClaimBlockRelayDecision(NodeId nodeid, uint64_t expected_generation) EXCLUSIVE_LOCKS_REQUIRED(!m_block_relay_mutex);
    bool TryClaimBlockRelayEviction(NodeId nodeid, uint64_t expected_global_generation,
                                    const std::vector<std::pair<NodeId, uint64_t>>& expected_peer_generations) EXCLUSIVE_LOCKS_REQUIRED(!m_block_relay_mutex);
    bool TryAdvanceHighestFastAnnounce(int height) EXCLUSIVE_LOCKS_REQUIRED(!m_block_relay_mutex);
    void BumpBlockRelayPeerGeneration(BlockRelayState& state) EXCLUSIVE_LOCKS_REQUIRED(m_block_relay_mutex);
    ...
    int m_outbound_peers_with_protect_from_disconnect GUARDED_BY(m_block_relay_mutex) = 0;
    ...
    int m_highest_fast_announce GUARDED_BY(m_block_relay_mutex){0};
     ...
     */
    void MaybeSetPeerAsAnnouncingHeaderAndIDs(NodeId nodeid) EXCLUSIVE_LOCKS_REQUIRED(cs_main, !m_peer_mutex, !m_block_relay_mutex);

    /** Stack of nodes which we have set to announce using compact blocks */
    std::list<NodeId> lNodesAnnouncingHeaderAndIDs GUARDED_BY(m_block_relay_mutex);

    /** Changes on every mutation of a global block-relay collection/counter. */
    uint64_t m_block_relay_generation GUARDED_BY(m_block_relay_mutex){0};

    /** Monotonic source for unique block-relay peer-state generations. */
    uint64_t m_next_block_relay_peer_generation GUARDED_BY(m_block_relay_mutex){0};
...
void PeerManagerImpl::AddBlockRelayPeer(NodeId nodeid)
{
    LOCK(m_block_relay_mutex);
    auto [it, inserted]{m_node_states.try_emplace(nodeid)};
    assert(inserted);
    BumpBlockRelayPeerGeneration(it->second);
    ++m_block_relay_generation;
}

PeerManagerImpl::RemovedBlockRelayState PeerManagerImpl::RemoveBlockRelayPeer(NodeId nodeid)
{
    LOCK(m_block_relay_mutex);
    auto it{m_node_states.find(nodeid)};
    assert(it != m_node_states.end());
    const bool was_protected{it->second.m_chain_sync.m_protect};
    BumpBlockRelayPeerGeneration(it->second);
    m_outbound_peers_with_protect_from_disconnect -= was_protected;
    assert(m_outbound_peers_with_protect_from_disconnect >= 0);
    lNodesAnnouncingHeaderAndIDs.remove(nodeid);
    m_node_states.erase(it);
    ++m_block_relay_generation;
    const bool empty{m_node_states.empty()};
    if (empty) {
        assert(m_outbound_peers_with_protect_from_disconnect == 0);
        assert(lNodesAnnouncingHeaderAndIDs.empty());
    }
    return {.m_empty = empty};
}

std::optional<PeerManagerImpl::BlockRelayPeerSnapshot> PeerManagerImpl::GetBlockRelayPeerSnapshot(NodeId nodeid) const
{
    LOCK(m_block_relay_mutex);
    const auto it{m_node_states.find(nodeid)};
    if (it == m_node_states.end()) return std::nullopt;
    const BlockRelayState& state{it->second};
    return BlockRelayPeerSnapshot{
        .m_requested_hb_cmpctblocks = state.m_requested_hb_cmpctblocks,
        .m_provides_cmpctblocks = state.m_provides_cmpctblocks,
        .m_chain_sync = state.m_chain_sync,
        .m_last_block_announcement = state.m_last_block_announcement,
        .m_generation = state.m_generation,
    };
}

std::optional<PeerManagerImpl::BlockRelaySnapshot> PeerManagerImpl::GetBlockRelaySnapshot(NodeId nodeid) const
{
    LOCK(m_block_relay_mutex);
    const auto it{m_node_states.find(nodeid)};
    if (it == m_node_states.end()) return std::nullopt;
    const BlockRelayState& state{it->second};
    return BlockRelaySnapshot{
        .m_peer = {
            .m_requested_hb_cmpctblocks = state.m_requested_hb_cmpctblocks,
            .m_provides_cmpctblocks = state.m_provides_cmpctblocks,
            .m_chain_sync = state.m_chain_sync,
        ...
        .m_global = {
            ...
            .m_nodes_announcing_header_and_ids = lNodesAnnouncingHeaderAndIDs,
            .m_generation = m_block_relay_generation,
        },
    };
}

std::optional<PeerManagerImpl::BlockRelayPeerSnapshots> PeerManagerImpl::GetBlockRelayPeerSnapshots(const std::vector<NodeId>& nodeids) const
{
    LOCK(m_block_relay_mutex);
    BlockRelayPeerSnapshots snapshots{
        .m_peers = {},
        .m_global_generation = m_block_relay_generation,
    };
    snapshots.m_peers.reserve(nodeids.size());
    for (const NodeId nodeid : nodeids) {
        const auto it{m_node_states.find(nodeid)};
        if (it == m_node_states.end()) return std::nullopt;
        const BlockRelayState& state{it->second};
        snapshots.m_peers.emplace_back(nodeid, BlockRelayPeerSnapshot{
            .m_requested_hb_cmpctblocks = state.m_requested_hb_cmpctblocks,
            .m_provides_cmpctblocks = state.m_provides_cmpctblocks,
            .m_chain_sync = state.m_chain_sync,
            .m_last_block_announcement = state.m_last_block_announcement,
...
bool PeerManagerImpl::SetCompactBlockState(NodeId nodeid, bool high_bandwidth)
{
    LOCK(m_block_relay_mutex);
    const auto it{m_node_states.find(nodeid)};
    if (it == m_node_states.end()) return false;
    it->second.m_provides_cmpctblocks = true;
    it->second.m_requested_hb_cmpctblocks = high_bandwidth;
    BumpBlockRelayPeerGeneration(it->second);
    return true;
}

bool PeerManagerImpl::SetLastBlockAnnouncement(NodeId nodeid, int64_t time_in_seconds)
{
    LOCK(m_block_relay_mutex);
    const auto it{m_node_states.find(nodeid)};
    if (it == m_node_states.end()) return false;
    it->second.m_last_block_announcement = time_in_seconds;
    BumpBlockRelayPeerGeneration(it->second);
    return true;
}

bool PeerManagerImpl::TrySetChainSyncState(NodeId nodeid, uint64_t expected_generation,
                                            BlockRelayState::ChainSyncTimeoutState state)
{
    LOCK(m_block_relay_mutex);
    const auto it{m_node_states.find(nodeid)};
    if (it == m_node_states.end() || it->second.m_generation != expected_generation ||
        it->second.m_chain_sync.m_protect != state.m_protect) {
        return false;
    }
    it->second.m_chain_sync = std::move(state);
    BumpBlockRelayPeerGeneration(it->second);
    return true;
}

bool PeerManagerImpl::TryProtectPeer(NodeId nodeid, uint64_t expected_peer_generation,
                                     uint64_t expected_global_generation)
{
    LOCK(m_block_relay_mutex);
    const auto it{m_node_states.find(nodeid)};
    if (it == m_node_states.end() || it->second.m_generation != expected_peer_generation ||
        m_block_relay_generation != expected_global_generation ||
        it->second.m_chain_sync.m_protect ||
        m_outbound_peers_with_protect_from_disconnect >= MAX_OUTBOUND_PEERS_TO_PROTECT_FROM_DISCONNECT) {
        return false;
    }
    it->second.m_chain_sync.m_protect = true;
    BumpBlockRelayPeerGeneration(it->second);
...
bool PeerManagerImpl::TryUpdateNodesAnnouncingHeaderAndIDs(NodeId nodeid,
                                                            uint64_t expected_peer_generation,
                                                            uint64_t expected_global_generation,
                                                            std::list<NodeId> nodes)
{
    ...
    if (it == m_node_states.end() || it->second.m_generation != expected_peer_generation ||
        m_block_relay_generation != expected_global_generation ||
        !it->second.m_provides_cmpctblocks) {
        return false;
    }
    assert(nodes.size() <= 3);
    lNodesAnnouncingHeaderAndIDs = std::move(nodes);
    BumpBlockRelayPeerGeneration(it->second);
    ++m_block_relay_generation;
    return true;
}

bool PeerManagerImpl::TryClaimBlockRelayDecision(NodeId nodeid, uint64_t expected_generation)
{
    LOCK(m_block_relay_mutex);
    const auto it{m_node_states.find(nodeid)};
    if (it == m_node_states.end() || it->second.m_generation != expected_generation) return false;
    BumpBlockRelayPeerGeneration(it->second);
    return true;
}

bool PeerManagerImpl::TryClaimBlockRelayEviction(
    NodeId nodeid,
    uint64_t expected_global_generation,
    const std::vector<std::pair<NodeId, uint64_t>>& expected_peer_generations)
{
    LOCK(m_block_relay_mutex);
    if (m_block_relay_generation != expected_global_generation) return false;
    for (const auto& [expected_nodeid, expected_generation] : expected_peer_generations) {
        const auto it{m_node_states.find(expected_nodeid)};
        if (it == m_node_states.end() || it->second.m_generation != expected_generation) return false;
    }
    const auto selected{m_node_states.find(nodeid)};
    if (selected == m_node_states.end()) return false;
    BumpBlockRelayPeerGeneration(selected->second);
    return true;
}

bool PeerManagerImpl::TryAdvanceHighestFastAnnounce(int height)
{
    LOCK(m_block_relay_mutex);
    if (height <= m_highest_fast_announce) return false;
    m_highest_fast_announce = height;
    ++m_block_relay_generation;
    return true;
}

void PeerManagerImpl::BumpBlockRelayPeerGeneration(BlockRelayState& state)
{
    state.m_generation = ++m_next_block_relay_peer_generation;
}
...
void PeerManagerImpl::MaybeSetPeerAsAnnouncingHeaderAndIDs(NodeId nodeid)
{
    ...
    for (int attempt{0}; attempt < 2; ++attempt) {
        const auto relay{GetBlockRelaySnapshot(nodeid)};
        if (!relay || !relay->m_peer.m_provides_cmpctblocks) {
            // Don't request compact blocks if the peer has not signalled support
            return;
        }

        std::list<NodeId> nodes{relay->m_global.m_nodes_announcing_header_and_ids};
        const auto existing{std::ranges::find(nodes, nodeid)};
        if (existing != nodes.end()) {
            nodes.erase(existing);
            nodes.push_back(nodeid);
            if (TryUpdateNodesAnnouncingHeaderAndIDs(
                    nodeid, relay->m_peer.m_generation, relay->m_global.m_generation, std::move(nodes))) {
                return;
            }
            continue;
        }
        ...
        for (const NodeId id : nodes) {
            PeerRef peer_ref{GetPeerRef(id)};
            if (peer_ref && !peer_ref->m_is_inbound) ++num_outbound_hb_peers;
        }
        if (peer && peer->m_is_inbound) {
            // If we're adding an inbound HB peer, make sure we're not removing
            // our last outbound HB peer in the process.
            if (nodes.size() >= 3 && num_outbound_hb_peers == 1) {
                PeerRef remove_peer{GetPeerRef(nodes.front())};
                if (remove_peer && !remove_peer->m_is_inbound) {
                    // Put the HB outbound peer in the second slot, so that it
                    // doesn't get removed.
                    std::swap(nodes.front(), *std::next(nodes.begin()));
                }
            }
        }

        const bool sent_high_bandwidth{m_connman.ForNode(nodeid, [this](CNode* pfrom) {
            MakeAndPushMessage(*pfrom, NetMsgType::SENDCMPCT, /*high_bandwidth=*/true, /*version=*/CMPCTBLOCKS_VERSION);
            pfrom->m_bip152_highbandwidth_to = true;
            return true;
        })};
        if (!sent_high_bandwidth) return;

        nodes.push_back(nodeid);
        std::optional<NodeId> remove_node;
        if (nodes.size() > 3) {
            // As per BIP152, we only get 3 of our peers to announce blocks
            // using compact encodings.
            remove_node = nodes.front();
            nodes.pop_front();
        }
        if (TryUpdateNodesAnnouncingHeaderAndIDs(
                nodeid, relay->m_peer.m_generation, relay->m_global.m_generation, std::move(nodes))) {
            if (remove_node) {
                m_connman.ForNode(*remove_node, [this](CNode* pnode_stop) {
                    MakeAndPushMessage(*pnode_stop, NetMsgType::SENDCMPCT, /*high_bandwidth=*/false, /*version=*/CMPCTBLOCKS_VERSION);
                    pnode_stop->m_bip152_highbandwidth_to = false;
                    return true;
                });
            }
            return;
        ...
        // The relay state changed before the list update. Restore the wire
        // selection before retrying from a fresh snapshot.
        m_connman.ForNode(nodeid, [this](CNode* pfrom) {
            ...
            pfrom->m_bip152_highbandwidth_to = false;
            return true;
        });
    }
}
...
bool PeerManagerImpl::TipMayBeStale()
{
    ...
    return m_block_downloadman->TipMayBeStale(
        GetTime<std::chrono::seconds>(),
        std::chrono::seconds{consensusParams.nPowTargetSpacing * 3});
}
...
{
    SetLastBlockAnnouncement(node, time_in_seconds);
}

void PeerManagerImpl::InitializeNode(const CNode& node, ServiceFlags our_services)
{
    NodeId nodeid = node.GetId();
    AddBlockRelayPeer(nodeid);
    m_block_downloadman->ConnectedPeer(nodeid, {.m_is_inbound = node.IsInboundConn()});
...
void PeerManagerImpl::FinalizeNode(const CNode& node)
{
    ...
    m_block_downloadman->DisconnectedPeer(nodeid);
    {
        LOCK(m_tx_download_mutex);
        m_txdownloadman.DisconnectedPeer(nodeid);
    }
    if (m_txreconciliation) m_txreconciliation->ForgetPeer(nodeid);
    const RemovedBlockRelayState removed_relay_state{RemoveBlockRelayPeer(nodeid)};

    if (removed_relay_state.m_empty) {
        // Do a consistency check after the last peer is removed.
        assert(m_wtxid_relay_peers == 0);
        m_block_downloadman->CheckIsEmpty();
...
bool PeerManagerImpl::GetNodeStateStats(NodeId nodeid, CNodeStateStats& stats) const
{
    ...
    {
        if (!GetBlockRelayPeerSnapshot(nodeid)) return false;
        const auto request_info{m_block_downloadman->GetPeerSnapshot(nodeid)};
        if (!request_info) return false;
        stats.nSyncHeight = request_info->m_best_known_block ? request_info->m_best_known_block->m_height : -1;
        stats.nCommonHeight = request_info->m_last_common_block ? request_info->m_last_common_block->m_height : -1;
        for (const auto& block : request_info->m_blocks) stats.vHeightInFlight.push_back(block.m_height);
    }
...
util::Expected<void, std::string> PeerManagerImpl::FetchBlock(NodeId peer_id, const CBlockIndex& block_index)
{
    ...
    m_block_downloadman->RemoveBlockRequest(
        block_index.GetBlockHash(), std::nullopt, GetTime<std::chrono::microseconds>());

    // Mark block as in-flight
    const auto reservation{m_block_downloadman->ReserveBlockRequest(
        peer_id, MakeBlockDownloadBlock(block_index), GetTime<std::chrono::microseconds>())};
    if (reservation.m_status != node::BlockRequestStatus::NEW) {
        return util::Unexpected{"Already requested from this peer"};
    }
                                               ...
                                               Options opts)
{
    return std::make_unique<PeerManagerImpl>(
        connman, addrman, banman, chainman, pool, warnings, std::move(block_downloadman), opts);
}
...
PeerManagerImpl::PeerManagerImpl(CConnman& connman, AddrMan& addrman,
    ...
    : m_rng{opts.deterministic_rng},
      ...
      m_chainman(chainman),
      m_mempool(pool),
      m_block_downloadman{Assert(std::move(block_downloadman))},
    ...
    const CBlockIndex* pindex)
{
    ...
    m_block_downloadman->UpdatedBlockTip(GetTime<std::chrono::seconds>());

    // In case the dynamic timeout was doubled once or more, reduce it slowly back to its default value
    const auto stalling_timeout{m_block_downloadman->GetGlobalSnapshot().m_block_stalling_timeout};
    Assume(stalling_timeout >= node::DEFAULT_BLOCK_STALLING_TIMEOUT);
    if (const auto new_timeout{m_block_downloadman->TryDecreaseBlockStallingTimeout(stalling_timeout)}) {
...
void PeerManagerImpl::NewPoWValidBlock(const CBlockIndex *pindex, const std::shared_ptr<const CBlock>& pblock)
{
    ...
    if (!TryAdvanceHighestFastAnnounce(pindex->nHeight)) return;
    ...
    for (const NodeId nodeid : candidates) {
        for (int attempt{0}; attempt < 2; ++attempt) {
            const auto relay{GetBlockRelayPeerSnapshot(nodeid)};
            if (!relay) break;
            m_block_downloadman->ProcessBlockAvailability(nodeid);
            // If the peer has, or we announced to them the previous block
            // already, but we don't think they have this one, announce it.
            if (!relay->m_requested_hb_cmpctblocks ||
                m_block_downloadman->PeerHasHeader(nodeid, pindex->GetBlockHash()) ||
                !m_block_downloadman->PeerHasHeader(nodeid, pindex->pprev->GetBlockHash())) {
                break;
            }
            if (!TryClaimBlockRelayDecision(nodeid, relay->m_generation)) continue;

            const bool sent{m_connman.ForNode(nodeid, [&](CNode* pnode) {
                if (pnode->GetCommonVersion() < INVALID_CB_NO_BAN_VERSION || pnode->fDisconnect) return false;
                LogDebug(BCLog::NET, "%s sending header-and-ids %s to peer=%d\n", "PeerManager::NewPoWValidBlock",
                         hashBlock.ToString(), nodeid);
                const CSerializedNetMsg& ser_cmpctblock{lazy_ser.get()};
                PushMessage(*pnode, ser_cmpctblock.Copy());
                return true;
            })};
            if (sent) {
                m_block_downloadman->RecordBestHeaderSent(nodeid, MakeBlockDownloadBlock(*pindex));
            }
            break;
        }
    }
...
void PeerManagerImpl::BlockChecked(const std::shared_ptr<const CBlock>& block, const BlockValidationState& state)
{
    ...
    const auto source{m_block_downloadman->ConsumeBlockSource(hash)};

    // If the block failed validation, we know where it came from and we're still connected
    // to that peer, maybe punish.
    if (state.IsInvalid() && source) {
        PeerRef source_peer{GetPeerRef(source->m_peer)};
        if (source_peer && GetBlockRelayPeerSnapshot(source->m_peer)) {
            MaybePunishNodeForBlock(/*nodeid=*/ source->m_peer, state, /*via_compact_block=*/ !source->m_punish_on_invalid);
        }
    }
    // Check that:
    ...
    //    just check that there are currently no other blocks in flight.
             ...
             m_block_downloadman->AllRequestsAreFor(hash)) {
        if (source) {
            MaybeSetPeerAsAnnouncingHeaderAndIDs(source->m_peer);
        }
    }
}

void PeerManagerImpl::BlockDataAvailable(const uint256& hash) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    m_block_downloadman->RemoveBlockRequest(
        hash, std::nullopt, GetTime<std::chrono::microseconds>());
}
        ...
        const std::vector<CBlockHeader>& headers)
{
    ...
    // Record the unknown announcement for this peer, so that if we
    // eventually get the headers - even from a different peer -
    // we can use this peer to download.
    m_block_downloadman->UpdateBlockAvailability(pfrom.GetId(), headers.back().GetHash());
}
    ...
    if (CanDirectFetch() && last_header.IsValid(BLOCK_VALID_TREE) && m_chainman.ActiveChain().Tip()->nChainWork <= last_header.nChainWork) {
        ...
        while (pindexWalk && !m_chainman.ActiveChain().Contains(*pindexWalk) && vToFetch.size() <= MAX_BLOCKS_IN_TRANSIT_PER_PEER) {
            if (!(pindexWalk->nStatus & BLOCK_HAVE_DATA) &&
                    !m_block_downloadman->IsBlockRequested(pindexWalk->GetBlockHash()) &&
        ...
        if (!m_chainman.ActiveChain().Contains(*Assert(pindexWalk))) {
            ...
            for (const CBlockIndex* pindex : vToFetch | std::views::reverse) {
                const auto peer_requests{Assume(m_block_downloadman->GetPeerSnapshot(pfrom.GetId()))};
                if (peer_requests->m_blocks.size() >= MAX_BLOCKS_IN_TRANSIT_PER_PEER) {
                    // Can't download any more from this peer
                    break;
                }
                const auto reservation{m_block_downloadman->ReserveBlockRequest(
                    pfrom.GetId(), MakeBlockDownloadBlock(*pindex), GetTime<std::chrono::microseconds>())};
                if (!Assume(reservation.m_status == node::BlockRequestStatus::NEW)) continue;
            ...
            if (vGetData.size() > 0) {
                bool use_compact_block{false};
                for (int attempt{0}; attempt < 2; ++attempt) {
                    const auto relay{GetBlockRelayPeerSnapshot(pfrom.GetId())};
                    if (!relay) break;
                    const auto block_download{m_block_downloadman->GetGlobalSnapshot()};
                    if (m_opts.ignore_incoming_txs || !relay->m_provides_cmpctblocks ||
                        vGetData.size() != 1 || block_download.m_total_requests != 1 ||
                        !last_header.pprev->IsValid(BLOCK_VALID_CHAIN)) {
                        break;
                    }
                    if (TryClaimBlockRelayDecision(pfrom.GetId(), relay->m_generation)) {
                        use_compact_block = true;
                        break;
                    }
                }
        ...
        const CBlockIndex& last_header, bool received_new_header, bool may_have_more_headers)
{
    ...
    m_block_downloadman->UpdateBlockAvailability(pfrom.GetId(), last_header.GetBlockHash());

    // From here, the best-known block should be guaranteed to be present,
    // because it is set in UpdateBlockAvailability. Some empty checks
    // are still present, however, as belt-and-suspenders.

    if (received_new_header && last_header.nChainWork > m_chainman.ActiveChain().Tip()->nChainWork) {
        SetLastBlockAnnouncement(pfrom.GetId(), GetTime());
    }

    // If we're in IBD, we want outbound peers that will serve us a useful
    // chain. Disconnect peers that are on chains with insufficient work.
    const auto block_download{Assume(m_block_downloadman->GetPeerSnapshot(pfrom.GetId()))};
    if (m_chainman.IsInitialBlockDownload() && !may_have_more_headers) {
        // If the peer has no more headers to give us, then we know we have
        // their tip.
        if (block_download->m_best_known_block && block_download->m_best_known_block->m_chain_work < m_chainman.MinimumChainWork()) {
    ...
    if (!pfrom.fDisconnect && pfrom.IsFullOutboundConn()) {
        for (int attempt{0}; attempt < 2; ++attempt) {
            const auto relay{GetBlockRelaySnapshot(pfrom.GetId())};
            if (!relay) break;
            const auto protect_download{m_block_downloadman->GetPeerSnapshot(pfrom.GetId())};
            if (!protect_download || !protect_download->m_best_known_block ||
                relay->m_peer.m_chain_sync.m_protect ||
                relay->m_global.m_outbound_peers_with_protect_from_disconnect >= MAX_OUTBOUND_PEERS_TO_PROTECT_FROM_DISCONNECT ||
                protect_download->m_best_known_block->m_chain_work < m_chainman.ActiveChain().Tip()->nChainWork) {
                break;
            }
            if (TryProtectPeer(pfrom.GetId(), relay->m_peer.m_generation, relay->m_global.m_generation)) {
...
void PeerManagerImpl::ProcessBlock(CNode& node, const std::shared_ptr<const CBlock>& block, bool force_processing, bool min_pow_checked)
{
    ...
    if (new_block) {
        ...
        // The synchronous BlockDataAvailable notification already removed all
        // requests after BLOCK_HAVE_DATA publication. Retain this established
        // network-path cleanup as an idempotent backstop.
        // In case this block came from a different peer than we requested
        // from, we can erase the block request now anyway (as we just stored
        // this block to disk).
        m_block_downloadman->RemoveBlockRequest(
            block->GetHash(), std::nullopt, GetTime<std::chrono::microseconds>());
    } else {
        m_block_downloadman->EraseBlockSource(block->GetHash());
    }
}
...
void PeerManagerImpl::ProcessCompactBlockTxns(CNode& pfrom, Peer& peer, const BlockTransactions& block_transactions)
{
    ...
    bool fBlockRead{false};
    {
        ...
        const auto in_flight{m_block_downloadman->GetBlockInFlightInfo(
            block_transactions.blockhash, pfrom.GetId())};
        if (!in_flight.m_requested_from_peer || !in_flight.m_partial_block) {
            LogDebug(BCLog::NET, "Peer %d sent us block transactions for block we weren't expecting\n", pfrom.GetId());
            return;
        }

        // This copied handle keeps reconstruction state alive if removal below
        // drops the manager's reference. Message processing serializes mutation.
        const auto partial_block{in_flight.m_partial_block};
        PartiallyDownloadedBlock& partialBlock{*partial_block};
        ...
        if (partialBlock.header.IsNull()) {
            ...
            m_block_downloadman->RemoveBlockRequest(
                block_transactions.blockhash, pfrom.GetId(), GetTime<std::chrono::microseconds>());
        ...
        if (status == READ_STATUS_INVALID) {
            m_block_downloadman->RemoveBlockRequest(
                block_transactions.blockhash, pfrom.GetId(), GetTime<std::chrono::microseconds>()); // Reset in-flight state in case Misbehaving does not result in a disconnect
            Misbehaving(peer, "invalid compact block/non-matching block transactions");
            return;
        } else if (status == READ_STATUS_FAILED) {
            if (in_flight.m_first_in_flight) {
            ...
            m_block_downloadman->RemoveBlockRequest(
                block_transactions.blockhash, pfrom.GetId(), GetTime<std::chrono::microseconds>()); // manager ownership is released
            fBlockRead = true;
            // Block source attribution is used for potentially punishing peers and
            // updating which peers send us compact blocks, so the race
            // between here and cs_main in ProcessNewBlock is fine.
            // BIP 152 permits peers to relay compact blocks after validating
            // the header only; we should not punish peers if the block turns
            // out to be invalid.
            m_block_downloadman->RecordBlockSource(
                block_transactions.blockhash, pfrom.GetId(), /*punish_on_invalid=*/false);
        }
                                     ...
                                     const std::atomic<bool>& interruptMsgProc)
{
    ...
    if (msg_type == NetMsgType::VERSION) {
        ...
        // Atomically update the block-download connection capabilities learned
        // during VERSION without resetting request or timing state.
        m_block_downloadman->ConnectedPeer(pfrom.GetId(), {
            .m_is_inbound = pfrom.IsInboundConn(),
            .m_preferred_download = (!pfrom.IsInboundConn() || pfrom.HasPermission(NetPermissionFlags::NoBan)) && !pfrom.IsAddrFetchConn() && CanServeBlocks(peer),
            .m_can_serve_witness = CanServeWitnesses(peer),
            .m_limited_peer = IsLimitedPeer(peer),
        });
    ...
    if (msg_type == NetMsgType::VERACK) {
...

        {
            ...
            m_txdownloadman.ConnectedPeer(pfrom.GetId(), node::TxDownloadConnectionInfo {
                .m_preferred = block_download->m_preferred_download,
    ...
    if (msg_type == NetMsgType::SENDCMPCT) {
        ...
        if (!SetCompactBlockState(pfrom.GetId(), sendcmpct_hb)) return;
    ...
    if (msg_type == NetMsgType::INV) {
        ...
        for (CInv& inv : vInv) {
            ...
            if (inv.IsMsgBlk()) {
                ...
                m_block_downloadman->UpdateBlockAvailability(pfrom.GetId(), inv.hash);
                if (!fAlreadyHave && !m_chainman.m_blockman.LoadingBlocks() && !m_block_downloadman->IsBlockRequested(inv.hash)) {
        ...
        if (best_block != nullptr) {
            ...
            const auto block_download{Assume(m_block_downloadman->GetPeerSnapshot(pfrom.GetId()))};
            if (block_download->m_sync_started || (!peer.m_inv_triggered_getheaders_before_sync && *best_block != m_last_block_inv_triggering_headers_sync)) {
                if (MaybeSendGetHeaders(pfrom, GetLocator(m_chainman.m_best_header), peer)) {
                    LogDebug(BCLog::NET, "getheaders (%d) %s to peer=%d\n",
                            m_chainman.m_best_header->nHeight, best_block->ToString(),
                            pfrom.GetId());
                }
                if (!block_download->m_sync_started) {
    ...
    if (msg_type == NetMsgType::GETHEADERS) {
        ...
        // the best header sent to be our tip.
        //
        ...
        m_block_downloadman->RecordBestHeaderSent(
            pfrom.GetId(), MakeBlockDownloadBlock(*(pindex ? pindex : m_chainman.ActiveChain().Tip())));
    ...
    if (msg_type == NetMsgType::CMPCTBLOCK)
    {
        ...
        {
        LOCK(cs_main);
        m_block_downloadman->UpdateBlockAvailability(pfrom.GetId(), pindex->GetBlockHash());
        ...
        const size_t already_in_flight{in_flight.m_request_count};
        const bool requested_block_from_this_peer{in_flight.m_requested_from_peer};
        ...
        if (pindex->nHeight <= m_chainman.ActiveChain().Height() + 2) {
            const auto peer_requests{Assume(m_block_downloadman->GetPeerSnapshot(pfrom.GetId()))};
            if ((already_in_flight < MAX_CMPCTBLOCKS_INFLIGHT_PER_BLOCK && peer_requests->m_blocks.size() < MAX_BLOCKS_IN_TRANSIT_PER_PEER) ||
                 requested_block_from_this_peer) {
                auto proposed_partial{std::make_shared<PartiallyDownloadedBlock>(&m_mempool)};
                const auto reservation{m_block_downloadman->ReserveBlockRequest(
                    pfrom.GetId(), MakeBlockDownloadBlock(*pindex), GetTime<std::chrono::microseconds>(),
                    std::move(proposed_partial))};
                if (reservation.m_status == node::BlockRequestStatus::DUPLICATE_COMPACT) {
                    // The block was already in flight using compact blocks from the same peer.
                    LogDebug(BCLog::NET, "Peer sent us compact block we were already syncing!\n");
                    return;
                }
                if (!Assume(reservation.m_status == node::BlockRequestStatus::NEW ||
                            reservation.m_status == node::BlockRequestStatus::UPGRADED_TO_COMPACT)) return;

                const auto partial_block{Assume(reservation.m_partial_block)};
                PartiallyDownloadedBlock& partialBlock{*partial_block};
                ReadStatus status = partialBlock.InitData(cmpctblock, vExtraTxnForCompact);
                if (status == READ_STATUS_INVALID) {
                    m_block_downloadman->RemoveBlockRequest(
                        pindex->GetBlockHash(), pfrom.GetId(), GetTime<std::chrono::microseconds>()); // Reset in-flight state in case Misbehaving does not result in a disconnect
                    Misbehaving(peer, "invalid compact block");
                    return;
                } else if (status == READ_STATUS_FAILED) {
                    if (reservation.m_first_in_flight)  {
                        ...
                        m_block_downloadman->RemoveBlockRequest(
                            pindex->GetBlockHash(), pfrom.GetId(), GetTime<std::chrono::microseconds>());
                    }
                    return;
                }
                ...
                if (req.indexes.empty()) {
                ...
                } else if (reservation.m_first_in_flight) {
                    ...
                    (!pfrom.IsInboundConn() ||
                    m_block_downloadman->IsBlockRequestedFromOutbound(blockhash) ||
        ...
        if (fBlockReconstructed) {
            ...
            m_block_downloadman->RecordBlockSource(
                pblock->GetHash(), pfrom.GetId(), /*punish_on_invalid=*/false);
            ...
            if (pindex->IsValid(BLOCK_VALID_TRANSACTIONS)) {
                ...
                m_block_downloadman->RemoveBlockRequest(
                    pblock->GetHash(), std::nullopt, GetTime<std::chrono::microseconds>());
            }
        }
        return;
    }
    ...
    if (msg_type == NetMsgType::BLOCK)
    {
        ...
        if (prev_block && IsBlockMutated(/*block=*/*pblock,
            ...
            m_block_downloadman->RemoveBlockRequest(
                pblock->GetHash(), peer.m_id, GetTime<std::chrono::microseconds>());
            return;
        }
        ...
        bool min_pow_checked = false;
        {
            ...
            const auto removal{m_block_downloadman->RemoveBlockRequest(
                hash, pfrom.GetId(), GetTime<std::chrono::microseconds>())};
            forceProcessing = removal.m_requests_before != 0;
            // Block source attribution is only used for punishing peers and setting
            // which peers send us compact blocks, so the race between here and
            // cs_main in ProcessNewBlock is fine.
            m_block_downloadman->RecordBlockSource(
                hash, pfrom.GetId(), /*punish_on_invalid=*/true);
...
void PeerManagerImpl::ConsiderEviction(CNode& pto, Peer& peer, std::chrono::seconds time_in_seconds)
{
    ...
    for (int attempt{0}; attempt < 2; ++attempt) {
        const auto relay{GetBlockRelayPeerSnapshot(pto.GetId())};
        if (!relay) return;
        const auto block_download{m_block_downloadman->GetPeerSnapshot(pto.GetId())};
        if (!block_download) return;

        if (relay->m_chain_sync.m_protect || !pto.IsOutboundOrBlockRelayConn() || !block_download->m_sync_started) {
            return;
        }

        auto chain_sync{relay->m_chain_sync};
        struct ChainTipSnapshot {
            node::BlockDownloadBlock m_tip;
            std::optional<CBlockLocator> m_locator;
        };
        ChainTipSnapshot tip_snapshot;
        {
            // Bounded validation snapshot: copy the active tip identity and
            // capture its predecessor locator only if a new benchmark may be needed.
            LOCK(cs_main);
            const CBlockIndex* tip{Assert(m_chainman.ActiveChain().Tip())};
            tip_snapshot = {
                .m_tip = MakeBlockDownloadBlock(*tip),
                .m_locator = std::nullopt,
            };
            const bool peer_caught_up{
                block_download->m_best_known_block &&
                block_download->m_best_known_block->m_chain_work >= tip->nChainWork};
            const bool needs_new_benchmark{
                !peer_caught_up &&
                (chain_sync.m_timeout == 0s ||
                 (chain_sync.m_work && block_download->m_best_known_block &&
                  block_download->m_best_known_block->m_chain_work >= chain_sync.m_work->m_block.m_chain_work))};
            if (needs_new_benchmark) tip_snapshot.m_locator = GetLocator(tip->pprev);
        }
        const auto tip_is_current = [&]() EXCLUSIVE_LOCKS_REQUIRED(::cs_main) {
            const CBlockIndex* tip{m_chainman.ActiveChain().Tip()};
            return tip && MakeBlockDownloadBlock(*tip) == tip_snapshot.m_tip;
        };
        const auto manager_state_is_current = [&]() EXCLUSIVE_LOCKS_REQUIRED(::cs_main) {
            const auto current{m_block_downloadman->GetPeerSnapshot(pto.GetId())};
            return current && current->m_generation == block_download->m_generation;
        };
        if (block_download->m_best_known_block &&
            block_download->m_best_known_block->m_chain_work >= tip_snapshot.m_tip.m_chain_work) {
            // The peer has sent a block with at least as much work as our
            // current tip, so clear an existing timeout.
            if (chain_sync.m_timeout == 0s) return;
            chain_sync.m_timeout = 0s;
            chain_sync.m_work.reset();
            chain_sync.m_sent_getheaders = false;
            bool committed{false};
            {
                // Revalidate the chain and manager views in the same bounded
                // transaction as the conditional relay-state commit.
                LOCK(cs_main);
                if (!tip_is_current() || !manager_state_is_current()) continue;
                committed = TrySetChainSyncState(pto.GetId(), relay->m_generation, std::move(chain_sync));
            }
            if (committed) return;
            continue;
        }

        if (chain_sync.m_timeout == 0s ||
            (chain_sync.m_work && block_download->m_best_known_block &&
             block_download->m_best_known_block->m_chain_work >= chain_sync.m_work->m_block.m_chain_work)) {
            // The peer is behind for the first time, or caught up to our old
            // benchmark while our tip advanced. Capture a new owned benchmark
            // and the exact locator the timeout path will later send.
            chain_sync.m_timeout = time_in_seconds + CHAIN_SYNC_TIMEOUT;
            chain_sync.m_work = ChainSyncBenchmark{
                .m_block = tip_snapshot.m_tip,
                .m_locator = *tip_snapshot.m_locator,
            };
            chain_sync.m_sent_getheaders = false;
            bool committed{false};
            {
                // Revalidate the owned benchmark and manager generation in the
                // same bounded transaction as the relay-state commit.
                LOCK(cs_main);
                if (!tip_is_current() || !manager_state_is_current()) continue;
                committed = TrySetChainSyncState(pto.GetId(), relay->m_generation, std::move(chain_sync));
            }
        ...
        if (chain_sync.m_timeout <= 0s || time_in_seconds <= chain_sync.m_timeout) return;

        if (chain_sync.m_sent_getheaders) {
            bool claimed{false};
            {
                // Bounded revalidation linearizes the disconnect decision
                // against the manager generation, active tip, and relay generation.
                LOCK(cs_main);
                if (!tip_is_current() || !manager_state_is_current()) continue;
                claimed = TryClaimBlockRelayDecision(pto.GetId(), relay->m_generation);
            }
            if (!claimed) continue;
            // They've run out of time to catch up.
            LogInfo("Outbound peer has old chain, best known block = %s, %s",
                    block_download->m_best_known_block ? block_download->m_best_known_block->m_hash.ToString() : "<none>",
                    pto.DisconnectMsg());
            pto.fDisconnect = true;
            return;
        }

        assert(chain_sync.m_work);
        const CBlockLocator locator{chain_sync.m_work->m_locator};
        const uint256 benchmark_hash{chain_sync.m_work->m_block.m_hash};
        chain_sync.m_sent_getheaders = true;
        // Bump the timeout to allow a response, which could clear or reset
        // the timeout, or result in disconnect if insufficient work arrives.
        chain_sync.m_timeout = time_in_seconds + HEADERS_RESPONSE_TIME;
        bool committed{false};
        {
            // Revalidate all three owned views in one bounded transaction; the
            // GETHEADERS send and logging happen after the locks are released.
            LOCK(cs_main);
            if (!tip_is_current() || !manager_state_is_current()) continue;
            committed = TrySetChainSyncState(pto.GetId(), relay->m_generation, std::move(chain_sync));
        }
        ...
        // A concurrent getheaders may suppress this send, as before; its
        // response still supplies the required chain-work evidence.
        MaybeSendGetHeaders(pto, locator, peer);
        LogDebug(BCLog::NET, "sending getheaders to outbound peer=%d to verify chain work (current best known block:%s, benchmark blockhash: %s)\n",
                 pto.GetId(), block_download->m_best_known_block ? block_download->m_best_known_block->m_hash.ToString() : "<none>",
                 benchmark_hash.ToString());
        return;
    }
}
...
void PeerManagerImpl::EvictExtraOutboundPeers(NodeClock::time_point now)
{
    ...
    if (m_connman.GetExtraBlockRelayCount() > 0) {
        struct BlockRelayCandidate {
            NodeId m_id;
            NodeClock::time_point m_connected;
            std::chrono::seconds m_last_block_time;
        };
        for (int attempt{0}; attempt < 2; ++attempt) {
            std::vector<BlockRelayCandidate> candidates;
            m_connman.ForEachNode([&](CNode* pnode) EXCLUSIVE_LOCKS_REQUIRED(::cs_main, m_connman.GetNodesMutex()) {
                if (!pnode->IsBlockOnlyConn() || pnode->fDisconnect) return;
                candidates.push_back({pnode->GetId(), pnode->m_connected, pnode->m_last_block_time});
            });
            std::ranges::sort(candidates, std::greater{}, &BlockRelayCandidate::m_id);
            if (candidates.size() < 2) break;

            const BlockRelayCandidate& youngest{candidates[0]};
            const BlockRelayCandidate& next_youngest{candidates[1]};
            // If our newest peer gave us a block more recently, consider the
            // second-newest peer instead.
            const BlockRelayCandidate candidate{
                youngest.m_last_block_time > next_youngest.m_last_block_time ? next_youngest : youngest};

            const auto relay{GetBlockRelayPeerSnapshot(candidate.m_id)};
            if (!relay) break;
            const auto request_info{m_block_downloadman->GetPeerSnapshot(candidate.m_id)};
            const bool disconnect{now - candidate.m_connected >= MINIMUM_CONNECT_TIME &&
                                  (!request_info || request_info->m_blocks.empty())};
            if (!disconnect) {
                LogDebug(BCLog::NET, "keeping block-relay-only peer=%d chosen for eviction (connect time: %d, blocks_in_flight: %d)\n",
                         candidate.m_id, TicksSinceEpoch<std::chrono::seconds>(candidate.m_connected),
                         request_info ? request_info->m_blocks.size() : 0);
                break;
            }
            if (!TryClaimBlockRelayDecision(candidate.m_id, relay->m_generation)) continue;

            const bool disconnected{m_connman.ForNode(candidate.m_id, [](CNode* pnode) {
                pnode->fDisconnect = true;
                return true;
            })};
            if (disconnected) {
                LogDebug(BCLog::NET, "disconnecting extra block-relay-only peer=%d (last block received at time %d)\n",
                         candidate.m_id, count_seconds(candidate.m_last_block_time));
            }
            break;
        }
    }
    ...
    if (m_connman.GetExtraFullOutboundCount() > 0) {
        ...
        struct FullRelayCandidate {
            NodeId m_id;
            NodeClock::time_point m_connected;
        };
        for (int attempt{0}; attempt < 2; ++attempt) {
            std::vector<FullRelayCandidate> candidates;
            m_connman.ForEachNode([&](CNode* pnode) EXCLUSIVE_LOCKS_REQUIRED(::cs_main, m_connman.GetNodesMutex()) {
                // Only consider outbound-full-relay peers that are not already
                // marked for disconnection and have another connection to the
                // same network.
                if (!pnode->IsFullOutboundConn() || pnode->fDisconnect ||
                    !m_connman.MultipleManualOrFullOutboundConns(pnode->addr.GetNetwork())) {
                    return;
                }
                candidates.push_back({pnode->GetId(), pnode->m_connected});
            });

            std::vector<NodeId> candidate_ids;
            candidate_ids.reserve(candidates.size());
            for (const FullRelayCandidate& candidate : candidates) candidate_ids.push_back(candidate.m_id);
            const auto relay_snapshots{GetBlockRelayPeerSnapshots(candidate_ids)};
            if (!relay_snapshots) break;

            std::optional<size_t> worst_index;
            for (size_t index{0}; index < candidates.size(); ++index) {
                const BlockRelayPeerSnapshot& relay{relay_snapshots->m_peers[index].second};
                if (relay.m_chain_sync.m_protect) continue;
                if (!worst_index ||
                    relay.m_last_block_announcement < relay_snapshots->m_peers[*worst_index].second.m_last_block_announcement ||
                    (relay.m_last_block_announcement == relay_snapshots->m_peers[*worst_index].second.m_last_block_announcement &&
                     candidates[index].m_id > candidates[*worst_index].m_id)) {
                    worst_index = index;
                }
            }
            if (!worst_index) break;
            const FullRelayCandidate& worst{candidates[*worst_index]};
            const BlockRelayPeerSnapshot& worst_relay{relay_snapshots->m_peers[*worst_index].second};

            const auto request_info{m_block_downloadman->GetPeerSnapshot(worst.m_id)};
            const bool disconnect{now - worst.m_connected > MINIMUM_CONNECT_TIME &&
                                  (!request_info || request_info->m_blocks.empty())};
            if (!disconnect) {
                LogDebug(BCLog::NET, "keeping outbound peer=%d chosen for eviction (connect time: %d, blocks_in_flight: %d)\n",
                         worst.m_id, TicksSinceEpoch<std::chrono::seconds>(worst.m_connected),
                         request_info ? request_info->m_blocks.size() : 0);
                break;
            }
            std::vector<std::pair<NodeId, uint64_t>> expected_generations;
            expected_generations.reserve(relay_snapshots->m_peers.size());
            for (const auto& [nodeid, relay] : relay_snapshots->m_peers) {
                expected_generations.emplace_back(nodeid, relay.m_generation);
            }
            if (!TryClaimBlockRelayEviction(
                    worst.m_id, relay_snapshots->m_global_generation, expected_generations)) {
                continue;
            }

            const bool disconnected{m_connman.ForNode(worst.m_id, [](CNode* pnode) {
                pnode->fDisconnect = true;
                return true;
            })};
            if (disconnected) {
                LogDebug(BCLog::NET, "disconnecting extra outbound peer=%d (last block announcement received at time %d)\n",
                         worst.m_id, worst_relay.m_last_block_announcement);
                // A successful extra-peer connection makes further extras
                // unnecessary until the next stale-tip event.
    ...
    if (now > m_stale_tip_check_time) {
        ...
        if (!m_chainman.m_blockman.LoadingBlocks() && m_connman.GetNetworkActive() && m_connman.GetUseAddrmanOutgoing() && TipMayBeStale()) {
            LogInfo("Potential stale tip detected, will try using extra outbound peer (last tip update: %d seconds ago)\n",
                      count_seconds(now - m_block_downloadman->GetGlobalSnapshot().m_last_tip_update));
...
void PeerManagerImpl::MaybeSendSendHeaders(CNode& node, Peer& peer)
{
    ...
    if (!peer.m_sent_sendheaders && node.GetCommonVersion() >= SENDHEADERS_VERSION) {
        const auto block_download{m_block_downloadman->GetPeerSnapshot(node.GetId())};
        if (block_download && block_download->m_best_known_block &&
            block_download->m_best_known_block->m_chain_work > m_chainman.MinimumChainWork()) {
...
bool PeerManagerImpl::SendMessages(CNode& node)
{
    ...
    {
        // Keep the chain/download send state local to this synchronous pass.
        // This is deliberately a plain lexical scope, not a lock region.
        const auto block_download{m_block_downloadman->GetPeerSnapshot(node.GetId())};
        if (!block_download) return false;

        // Determine whether we might try initial headers sync or parallel block
        // download from this peer. The manager values are one owned snapshot.
        bool sync_blocks_and_headers_from_peer{false};
        if (block_download->m_preferred_download) {
            ...
            // allow block downloads from this peer, too.
            ...
            if (block_download->m_num_preferred_download_peers == 0 ||
                block_download->m_total_requests == 0) {
                sync_blocks_and_headers_from_peer = true;
            }
        }
        ...
        struct InitialHeadersSyncSnapshot {
            bool m_send{false};
            CBlockLocator m_locator;
            int m_start_height{-1};
            node::BlockDownloadBlock m_best_header;
            NodeSeconds m_best_header_time{};
        } initial_headers;
        const auto headers_check_time{NodeClock::now()};
        {
            // Bounded validation snapshot: initialize and copy only the best-header
            // locator, height, and time needed by initial headers synchronization.
            LOCK(cs_main);
            if (m_chainman.m_best_header == nullptr) {
                m_chainman.m_best_header = m_chainman.ActiveChain().Tip();
            }
            const CBlockIndex* best_header{Assert(m_chainman.m_best_header)};
            initial_headers.m_best_header = MakeBlockDownloadBlock(*best_header);
            initial_headers.m_best_header_time = best_header->Time();
            if (!block_download->m_sync_started && CanServeBlocks(peer) &&
                !m_chainman.m_blockman.LoadingBlocks() &&
                ((block_download->m_num_sync_started == 0 && sync_blocks_and_headers_from_peer) ||
                 best_header->Time() > headers_check_time - 24h)) {
                // Starting at the preceding block ensures an up-to-date peer
                // returns a non-empty list that can initialize its known best block.
                const CBlockIndex* start{best_header->pprev ? best_header->pprev : best_header};
                initial_headers.m_send = true;
                initial_headers.m_locator = GetLocator(start);
                initial_headers.m_start_height = start->nHeight;
            }
        }
        if (initial_headers.m_send) {
            bool decision_is_current{false};
            {
                // Revalidate the best-header identity, peer generation, and all
                // manager counters used for eligibility in one bounded transaction.
                // GETHEADERS and StartSync remain outside cs_main.
                LOCK(cs_main);
                const CBlockIndex* current{m_chainman.m_best_header};
                const auto current_download{m_block_downloadman->GetPeerSnapshot(node.GetId())};
                decision_is_current =
                    current && MakeBlockDownloadBlock(*current) == initial_headers.m_best_header &&
                    !m_chainman.m_blockman.LoadingBlocks() &&
                    current_download && current_download->m_generation == block_download->m_generation &&
                    current_download->m_sync_started == block_download->m_sync_started &&
                    current_download->m_preferred_download == block_download->m_preferred_download &&
                    current_download->m_num_preferred_download_peers == block_download->m_num_preferred_download_peers &&
                    current_download->m_num_sync_started == block_download->m_num_sync_started &&
                    current_download->m_total_requests == block_download->m_total_requests;
            }
            initial_headers.m_send = decision_is_current;
        }
        if (initial_headers.m_send && MaybeSendGetHeaders(node, initial_headers.m_locator, peer)) {
            LogDebug(BCLog::NET, "initial getheaders (%d) to peer=%d", initial_headers.m_start_height, node.GetId());
            // FinalizeNode may remove the manager peer after the unlocked send.
            // Existing disconnect cleanup then owns all bookkeeping removal.
            if (m_block_downloadman->StartSync(node.GetId())) {
                peer.m_headers_sync_timeout = current_time + HEADERS_DOWNLOAD_TIMEOUT_BASE +
                                              (
                                                  // Convert HEADERS_DOWNLOAD_TIMEOUT_PER_HEADER to microseconds before scaling
                                                  // to maintain precision
                                                  std::chrono::microseconds{HEADERS_DOWNLOAD_TIMEOUT_PER_HEADER} *
                                                  Ticks<std::chrono::seconds>(NodeClock::now() - initial_headers.m_best_header_time) / consensusParams.nPowTargetSpacing);
            }
        }
        ...
        //
        const bool have_block_announcements{WITH_LOCK(
            peer.m_block_inv_mutex, return !peer.m_blocks_for_headers_relay.empty())};
        if (!have_block_announcements) {
            // Preserve availability processing even when there is nothing to announce.
            m_block_downloadman->ProcessBlockAvailability(node.GetId());
        } else {
            struct AnnouncementBlockSnapshot {
                node::BlockDownloadBlock m_block;
                CBlockHeader m_header;
                FlatFilePos m_position;
                bool m_in_active_chain{false};
            };
            struct AnnouncementChainSnapshot {
                uint256 m_active_tip;
                std::vector<AnnouncementBlockSnapshot> m_blocks;
            };
            enum class Announcement { NONE,
                                      COMPACT_BLOCK,
                                      HEADERS };

            for (int attempt{0}; attempt < 2; ++attempt) {
                const auto relay_state{GetBlockRelayPeerSnapshot(node.GetId())};
                if (!relay_state) return false;

                // Copy the current queue prefix before unlocked availability
                // processing. UpdatedBlockTip may append, and those later entries
                // must remain queued when this prefix is committed.
                const std::vector<uint256> queued_hashes{WITH_LOCK(
                    peer.m_block_inv_mutex, return peer.m_blocks_for_headers_relay)};
                if (queued_hashes.empty()) break;

                // Resolve pending availability without caller-held validation or
                // relay locks, then use its peer generation to reject mixed views.
                m_block_downloadman->ProcessBlockAvailability(node.GetId());
                const auto availability{m_block_downloadman->GetPeerSnapshot(node.GetId())};
                if (!availability) return false;

                // If we have no more than MAX_BLOCKS_TO_ANNOUNCE in our list and
                // the peer wants header announcements, plan the connecting suffix.
                bool revert_to_inv{(!peer.m_prefers_headers &&
                                    (!relay_state->m_requested_hb_cmpctblocks || queued_hashes.size() > 1)) ||
                                   queued_hashes.size() > MAX_BLOCKS_TO_ANNOUNCE};
                const std::span<const uint256> chain_hashes{
                    revert_to_inv ? std::span<const uint256>{queued_hashes}.last(1) : std::span<const uint256>{queued_hashes}};
                AnnouncementChainSnapshot chain_snapshot;
                chain_snapshot.m_blocks.reserve(chain_hashes.size());
                {
                    // Bounded validation snapshot: at most eight connecting
                    // announcement entries, or only the fallback INV tip.
                    LOCK(cs_main);
                    chain_snapshot.m_active_tip = Assert(m_chainman.ActiveChain().Tip())->GetBlockHash();
                    for (const uint256& hash : chain_hashes) {
                        const CBlockIndex* index{m_chainman.m_blockman.LookupBlockIndex(hash)};
                        assert(index);
                        chain_snapshot.m_blocks.push_back({
                            .m_block = MakeBlockDownloadBlock(*index),
                            .m_header = index->GetBlockHeader(),
                            .m_position = index->GetBlockPos(),
                            .m_in_active_chain = m_chainman.ActiveChain()[index->nHeight] == index,
                        });
                    }
                }

                std::vector<CBlock> headers;
                std::optional<AnnouncementBlockSnapshot> best_header;
                if (!revert_to_inv) {
                    bool found_starting_header{false};
                    for (const auto& announced : chain_snapshot.m_blocks) {
                        if (!announced.m_in_active_chain) {
                            // Bail out if we reorged away from this block.
                            revert_to_inv = true;
                            break;
                        }
                        if (best_header && announced.m_header.hashPrevBlock != best_header->m_block.m_hash) {
                            // Robustly handle a non-connecting announcement list.
                            revert_to_inv = true;
                            break;
                        }
                        best_header = announced;
                        if (found_starting_header) {
                            headers.emplace_back(announced.m_header);
                        } else if (m_block_downloadman->PeerHasHeader(node.GetId(), announced.m_block.m_hash)) {
                            continue;
                        } else if (announced.m_header.hashPrevBlock.IsNull() ||
                                   m_block_downloadman->PeerHasHeader(node.GetId(), announced.m_header.hashPrevBlock)) {
                ...
                Announcement announcement{Announcement::NONE};
                std::optional<node::BlockDownloadBlock> best_header_sent;
                std::optional<FlatFilePos> compact_block_position;
                std::optional<uint256> inv_hash;
                std::optional<uint256> reverting_inv_hash;
                std::optional<uint256> off_chain_inv_tip;

                if (!revert_to_inv && !headers.empty()) {
                    Assume(best_header.has_value());
                    if (headers.size() == 1 && relay_state->m_requested_hb_cmpctblocks) {
                        // We only send up to one block as header-and-ids.
                        announcement = Announcement::COMPACT_BLOCK;
                        best_header_sent = best_header->m_block;
                        compact_block_position = best_header->m_position;
                    } else if (peer.m_prefers_headers) {
                        announcement = Announcement::HEADERS;
                        best_header_sent = best_header->m_block;
                    } else {
                        revert_to_inv = true;
                    }
                ...
                if (revert_to_inv) {
                    const auto& fallback{chain_snapshot.m_blocks.back()};
                    reverting_inv_hash = fallback.m_block.m_hash;
                    if (!fallback.m_in_active_chain) off_chain_inv_tip = chain_snapshot.m_active_tip;
                    if (!m_block_downloadman->PeerHasHeader(node.GetId(), fallback.m_block.m_hash)) {
                        inv_hash = fallback.m_block.m_hash;
                    }
                }

                std::shared_ptr<const CBlockHeaderAndShortTxIDs> cached_compact_block;
                bool claimed{false};
                {
                    // Revalidate the manager generation observed by every
                    // PeerHasHeader call together with the copied active-chain facts,
                    // disk positions, and relay-generation claim.
                    LOCK(cs_main);
                    const auto current_availability{m_block_downloadman->GetPeerSnapshot(node.GetId())};
                    if (!current_availability || current_availability->m_generation != availability->m_generation) continue;
                    if (Assert(m_chainman.ActiveChain().Tip())->GetBlockHash() != chain_snapshot.m_active_tip) continue;
                    bool chain_matches{true};
                    for (const auto& announced : chain_snapshot.m_blocks) {
                        const CBlockIndex* current{m_chainman.m_blockman.LookupBlockIndex(announced.m_block.m_hash)};
                        if (!current || MakeBlockDownloadBlock(*current) != announced.m_block ||
                            current->GetBlockHeader().hashPrevBlock != announced.m_header.hashPrevBlock ||
                            current->GetBlockPos() != announced.m_position ||
                            (m_chainman.ActiveChain()[current->nHeight] == current) != announced.m_in_active_chain) {
                            chain_matches = false;
                            break;
                        }
                    }
                    if (!chain_matches) continue;
                    if (announcement == Announcement::COMPACT_BLOCK) {
                        LOCK(m_most_recent_block_mutex);
                        if (m_most_recent_block_hash == best_header_sent->m_hash) {
                            cached_compact_block = m_most_recent_compact_block;
                        }
                    }
                    claimed = TryClaimBlockRelayDecision(node.GetId(), relay_state->m_generation);
                }
                if (!claimed) continue;

                const auto queue_has_prefix = [&] {
                    LOCK(peer.m_block_inv_mutex);
                    return peer.m_blocks_for_headers_relay.size() >= queued_hashes.size() &&
                           std::ranges::equal(queued_hashes,
                                              std::span{peer.m_blocks_for_headers_relay}.first(queued_hashes.size()));
                };
                // UpdatedBlockTip may append concurrently, but the captured prefix
                // must remain intact. If not, retain it and defer without a send.
                if (!queue_has_prefix()) break;

                if (off_chain_inv_tip) {
                    LogDebug(BCLog::NET, "Announcing block %s not on main chain (tip=%s)\n",
                             reverting_inv_hash->ToString(), off_chain_inv_tip->ToString());
                }
                if (announcement == Announcement::COMPACT_BLOCK) {
                    CSerializedNetMsg message;
                    if (cached_compact_block) {
                        message = NetMsg::Make(NetMsgType::CMPCTBLOCK, *cached_compact_block);
                    } else {
                        CBlock block;
                        if (!m_chainman.m_blockman.ReadBlock(block, *compact_block_position, best_header_sent->m_hash)) {
                            // Pruning can win after validation revalidation. Keep
                            // the announcement prefix and retry on a later cycle.
                            break;
                        }
                        message = NetMsg::Make(
                            NetMsgType::CMPCTBLOCK,
                            CBlockHeaderAndShortTxIDs{block, m_rng.rand64()});
                    }
                    LogDebug(BCLog::NET, "%s sending header-and-ids %s to peer=%d\n", __func__,
                             headers.front().GetHash().ToString(), node.GetId());
                    PushMessage(node, std::move(message));
                    m_block_downloadman->RecordBestHeaderSent(node.GetId(), *best_header_sent);
                } else if (announcement == Announcement::HEADERS) {
                    CSerializedNetMsg message{NetMsg::Make(NetMsgType::HEADERS, TX_WITH_WITNESS(headers))};
                    if (headers.size() > 1) {
                        LogDebug(BCLog::NET, "%s: %u headers, range (%s, %s), to peer=%d\n", __func__,
                                 headers.size(), headers.front().GetHash().ToString(),
                                 headers.back().GetHash().ToString(), node.GetId());
                    } else {
                ...
                {
                    LOCK(peer.m_block_inv_mutex);
                    if (peer.m_blocks_for_headers_relay.size() >= queued_hashes.size() &&
                        std::ranges::equal(queued_hashes,
                                           std::span{peer.m_blocks_for_headers_relay}.first(queued_hashes.size()))) {
                        if (inv_hash) peer.m_blocks_for_inv_relay.push_back(*inv_hash);
                        peer.m_blocks_for_headers_relay.erase(
                            peer.m_blocks_for_headers_relay.begin(),
                            peer.m_blocks_for_headers_relay.begin() + queued_hashes.size());
                        queue_committed = true;
                    }
                }
                if (queue_committed && inv_hash) {
                    LogDebug(BCLog::NET, "%s: sending inv peer=%d hash=%s\n", __func__,
                             node.GetId(), inv_hash->ToString());
                }
                break;
            }
        }
        ...
        //
        std::vector<CInv> vInv;
        std::vector<uint256> block_inv_hashes;
        {
            LOCK(peer.m_block_inv_mutex);
            block_inv_hashes.swap(peer.m_blocks_for_inv_relay);
        }
        vInv.reserve(std::max<size_t>(block_inv_hashes.size(), INVENTORY_BROADCAST_TARGET));
        for (const uint256& hash : block_inv_hashes) {
        ...
        const auto request_info{m_block_downloadman->GetPeerSnapshot(node.GetId())};
        if (!request_info) return false;

        // Detect whether we're stalling
        const auto stalling_timeout{m_block_downloadman->GetGlobalSnapshot().m_block_stalling_timeout};
        if (request_info->m_stalling_since.count() && request_info->m_stalling_since < current_time - stalling_timeout) {
            ...
            if (const auto new_timeout{m_block_downloadman->TryIncreaseBlockStallingTimeout(stalling_timeout)}) {
        ...
        if (!request_info->m_blocks.empty()) {
            const int nOtherPeersWithValidatedDownloads{request_info->m_peers_downloading_from - 1};
            if (current_time > request_info->m_downloading_since + std::chrono::seconds{consensusParams.nPowTargetSpacing} * (BLOCK_DOWNLOAD_TIMEOUT_BASE + BLOCK_DOWNLOAD_TIMEOUT_PER_PEER * nOtherPeersWithValidatedDownloads)) {
                LogInfo("Timeout downloading block %s, %s", request_info->m_blocks.front().m_hash.ToString(), node.DisconnectMsg());
                node.fDisconnect = true;
                return true;
            }
        }
        // Check for headers sync timeouts.
        if (request_info->m_sync_started && peer.m_headers_sync_timeout < std::chrono::microseconds::max()) {
            struct BestHeaderSnapshot {
                node::BlockDownloadBlock m_block;
                NodeSeconds m_time;
            } best_header;
            {
                // Bounded validation snapshot: copy only the best-header identity
                // and time consumed by the initial-headers timeout decision.
                LOCK(cs_main);
                const CBlockIndex* current{Assert(m_chainman.m_best_header)};
                best_header = {.m_block = MakeBlockDownloadBlock(*current), .m_time = current->Time()};
            }
            if (best_header.m_time <= NodeClock::now() - 24h) {
                if (current_time > peer.m_headers_sync_timeout && request_info->m_num_sync_started == 1 &&
                    (request_info->m_num_preferred_download_peers - request_info->m_preferred_download >= 1)) {
                    bool decision_is_current{false};
                    {
                        // Revalidate the manager generation and global eligibility
                        // counters in the same bounded transaction as the best header.
                        // Disconnect, logging, and manager updates remain outside.
                        LOCK(cs_main);
                        const CBlockIndex* current{m_chainman.m_best_header};
                        const auto current_request{m_block_downloadman->GetPeerSnapshot(node.GetId())};
                        decision_is_current =
                            current && MakeBlockDownloadBlock(*current) == best_header.m_block &&
                            current_request && current_request->m_generation == request_info->m_generation &&
                            current_request->m_sync_started == request_info->m_sync_started &&
                            current_request->m_preferred_download == request_info->m_preferred_download &&
                            current_request->m_num_sync_started == request_info->m_num_sync_started &&
                            current_request->m_num_preferred_download_peers == request_info->m_num_preferred_download_peers;
                    }
                    if (decision_is_current) {
                        ...
                        if (!node.HasPermission(NetPermissionFlags::NoBan)) {
                            ...
                            if (m_block_downloadman->ClearSync(node.GetId())) {
                ...
                // disconnect later. Revalidate the manager generation and owned
                // best-header identity before the observable peer timeout change.
                bool decision_is_current{false};
                {
                    LOCK(cs_main);
                    const CBlockIndex* current{m_chainman.m_best_header};
                ...
                if (decision_is_current) peer.m_headers_sync_timeout = std::chrono::microseconds::max();
            }
        }

        // Check that outbound peers have reasonable chains. GetTime() is used by
        // this anti-DoS logic so we can test it using mocktime.
        ...
        //
        std::vector<CInv> vGetData;
        if (CanServeBlocks(peer) && ((sync_blocks_and_headers_from_peer && !IsLimitedPeer(peer)) || !m_chainman.IsInitialBlockDownload()) && request_info->m_blocks.size() < MAX_BLOCKS_IN_TRANSIT_PER_PEER) {
            const node::BlockDownloadBatch download_batch{m_block_downloadman->PlanAndReserve(
                node.GetId(),
                MAX_BLOCKS_IN_TRANSIT_PER_PEER - request_info->m_blocks.size(),
                current_time,
                /*allow_historical=*/!IsLimitedPeer(peer))};
            if (download_batch.m_assumeutxo_blocked) {
            ...
            for (const auto& block : download_batch.m_blocks) {
                vGetData.emplace_back(MSG_BLOCK | fetch_flags, block.m_hash);
                LogDebug(BCLog::NET, "Requesting block %s (%d) peer=%d\n", block.m_hash.ToString(),
                         block.m_height, node.GetId());
            }
            if (download_batch.m_staller) {
                LogDebug(BCLog::NET, "Stall started peer=%d\n", *download_batch.m_staller);
            }
        }
        ...
        //
        std::vector<GenTxid> tx_requests;
        {
            LOCK(m_tx_download_mutex);
            tx_requests = m_txdownloadman.GetRequestsToSend(node.GetId(), current_time);
        }
