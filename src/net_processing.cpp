...
// Internal stuff
namespace {
bool IsP2PBlockMessage(std::string_view msg_type)
{
...
private:
    ...
    */
    ...
    void ProcessP2PBlockCompletion(
        NodeId source,
        const uint256& hash,
        bool new_block)
        EXCLUSIVE_LOCKS_REQUIRED(NetEventsInterface::g_msgproc_mutex);
                                               ...
                                               std::unique_ptr<node::P2PBlockValidation> block_validation)
{
    ...
    return std::make_unique<PeerManagerImpl>(connman, addrman, banman, chainman, pool, warnings, opts, std::move(block_validation));
}
    ...
    P2PBlockOrigin origin)
{
    ...
    case node::P2PBlockValidationSubmit::ACCEPTED:
        ...
        ProcessP2PBlockValidationResult();
        return;
    case node::P2PBlockValidationSubmit::BUSY:
        Assume(false);
        break;
...
void PeerManagerImpl::ProcessP2PBlockValidationResult()
{
    ...
    if (pending.origin == P2PBlockOrigin::CMPCTBLOCK) {
        ...
        if (pindex->IsValid(BLOCK_VALID_TRANSACTIONS)) {
            ...
            // some other peer. Doing this after ProcessNewBlock prevents a
            // malleated cmpctblock announcement from interfering with relay.
                                     ...
                                     const std::atomic<bool>& interruptMsgProc)
{
    ...
    if (msg_type == NetMsgType::BLOCK)
    {
        ...
        StartP2PBlockValidation(
            pfrom.GetId(), pblock, forceProcessing, min_pow_checked,
...
bool PeerManagerImpl::ProcessMessages(CNode& node, std::atomic<bool>& interruptMsgProc)
{
    ...
    // and prevents m_getdata_requests to grow unbounded
    {
        LOCK(peer.m_getdata_requests_mutex);
        if (!peer.m_getdata_requests.empty()) {
            return !P2PBlockValidationActive();
        }
    }
    ...
    if (P2PBlockValidationActive()) {
        const std::optional<std::string> next_type{node.PeekMessageType()};
        if (!next_type || IsP2PBlockMessage(*next_type) || *next_type == NetMsgType::GETDATA) {
            return false;
        }
    }
