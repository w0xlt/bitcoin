      ...
      m_opts{opts}
{
    {
        LOCK(::cs_main);
        m_stale_tips.Initialize(m_chainparams.GetChainType(), m_chainman.m_blockman, m_chainman.ActiveChain());
    }
...
void PeerManagerImpl::MaybeSendStaleTips(CNode& node, Peer& peer, CNodeState& state)
{
    ...
    std::erase_if(peer.m_stale_tip_announced_seqnos, [&](uint32_t seqno) { return !current_seqnos.contains(seqno); });
}
