...
private:
    ...
    */
     ...
     * about and we fully-validated them at some point. When stale-tip relay
     * is enabled, tracked stale tips whose block data is available are also
     * allowed, even though they were never fully validated, so that peers can
     * fetch the blocks announced via `staletip` messages.
     */
...
bool PeerManagerImpl::BlockRequestAllowed(const CBlockIndex& block_index)
{
    ...
    // Tracked stale tips never connect to the active chain and so never reach
    // BLOCK_VALID_SCRIPTS, but peers receiving our `staletip` announcements
    // with `have_block` set must be able to fetch the tip block. Serving them
    // is bounded by the cache's recency, fork-length and work eligibility
    // rules.
    if (m_opts.stale_tip_mode != StaleTipMode::NONE &&
        m_stale_tips.CanServeStaleTipBlock(m_chainman.ActiveChain(), &block_index)) {
        return true;
    }
    ...
    if (msg_type == NetMsgType::STALETIP) {
        ...
        const arith_uint256 anti_dos_work_threshold{GetAntiDoSWorkThreshold()};
        {
            ...
            if (known_tip != nullptr) {
                const bool tip_has_more_work{active_tip != nullptr && known_tip->nChainWork > active_tip->nChainWork};
                if (stale_tip_data.m_have_block && tip_has_more_work) {
                    UpdatePeerStateForReceivedHeaders(pfrom, peer, *known_tip, /*received_new_header=*/false, /*may_have_more_headers=*/false);
                    HeadersDirectFetchBlocks(pfrom, peer, *known_tip);
                }
        ...
        const bool tip_has_more_work{pindexLast != nullptr && WITH_LOCK(cs_main, return m_chainman.ActiveTip() != nullptr &&
                                                                                       pindexLast->nChainWork > m_chainman.ActiveTip()->nChainWork)};
        if (stale_tip_data.m_have_block && tip_has_more_work) {
            UpdatePeerStateForReceivedHeaders(pfrom, peer, *pindexLast, /*received_new_header=*/true, /*may_have_more_headers=*/false);
            HeadersDirectFetchBlocks(pfrom, peer, *pindexLast);
        }
...
void PeerManagerImpl::MaybeSendStaleTips(CNode& node, Peer& peer, CNodeState& state)
{
    ...
    for (const auto& announcement : m_stale_tips.GetTipsToAnnounce(m_chainman.ActiveChain(), want_blocks)) {
        ...
        if (announcement.fork.fork_point->nHeight > state.pindexLastCommonBlock->nHeight) continue;
        ...
        StaleTipData data{announcement.fork, have_block};
...
bool PeerManagerImpl::HandleStaleTip(CNode& pfrom, Peer& peer, const CBlockIndex* pindex, bool peer_has_block)
{
    ...
    if (!added && !can_request) return false;

    if (!peer_has_block) return false;
    if (!can_request) return false;
    if (m_opts.stale_tip_mode != StaleTipMode::BLOCKS) return false;
    if (!CanServeBlocks(peer)) return false;
    if (!CanServeWitnesses(peer) &&
        DeploymentActiveAt(*pindex, m_chainman, Consensus::DEPLOYMENT_SEGWIT)) return false;
