...
private:
    ...
    */
     ...
     * is enabled, blocks on tracked stale branches whose tip block data is
     * available are also allowed, even though they were never fully validated,
     * so that peers can fetch the blocks announced via `staletip` messages.
     */
...
bool PeerManagerImpl::BlockRequestAllowed(const CBlockIndex& block_index)
{
    ...
    // Tracked stale branches never connect to the active chain and so never
    // reach BLOCK_VALID_SCRIPTS, but peers receiving our `staletip`
    // announcements with `have_block` set must be able to fetch the announced
    // branch. Serving them is bounded by the cache's recency, fork-length and
...
bool PeerManagerImpl::HandleStaleTip(CNode& pfrom, Peer& peer, const CBlockIndex* pindex, bool peer_has_block)
{
    ...
    for (const CBlockIndex* block{pindex}; block != nullptr && !m_chainman.ActiveChain().Contains(*block); block = block->pprev) {
        ...
        BlockRequested(pfrom.GetId(), *block);
