...
CBlockIndex* AddBlockIndex(node::BlockManager& blockman, CBlockIndex*& best_header, const CBlockIndex* prev, uint32_t nonce)
{
    CBlockHeader header;
    header.nVersion = 4;
    header.hashPrevBlock = prev ? prev->GetBlockHash() : uint256::ZERO;
...
BOOST_AUTO_TEST_CASE(staletip_cache_uses_free_slots_and_evicts_lowest_tip)
{
    ...
    const auto is_tracked = [&](const CBlockIndex* block) {
