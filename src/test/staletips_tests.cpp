...
{
    CBlockHeader fork_header;
    fork_header.nVersion = 1;
    fork_header.hashMerkleRoot = uint256{uint8_t{1}};
    fork_header.nTime = 1;
    fork_header.nBits = 2;
    fork_header.nNonce = 3;
    const uint256 fork_hash{fork_header.GetHash()};
    CBlockIndex fork_point{fork_header};
    fork_point.phashBlock = &fork_hash;
    fork_point.nHeight = 0;

    CBlockHeader first_header;
    first_header.nVersion = 4;
    first_header.hashPrevBlock = fork_hash;
    first_header.hashMerkleRoot = uint256{uint8_t{2}};
    first_header.nTime = 5;
    first_header.nBits = 6;
    first_header.nNonce = 7;
    const uint256 first_hash{first_header.GetHash()};
    CBlockIndex first{first_header};
    first.phashBlock = &first_hash;
    first.pprev = &fork_point;
    first.nHeight = 1;
    first.BuildSkip();

    CBlockHeader tip_header;
    tip_header.nVersion = 8;
    tip_header.hashPrevBlock = first_hash;
    tip_header.hashMerkleRoot = uint256{uint8_t{3}};
    tip_header.nTime = 9;
    tip_header.nBits = 10;
    tip_header.nNonce = 11;
    const uint256 tip_hash_expected{tip_header.GetHash()};
    CBlockIndex tip{tip_header};
    tip.phashBlock = &tip_hash_expected;
    tip.pprev = &first;
    tip.nHeight = 2;
    tip.BuildSkip();

    LOCK(cs_main);
    const StaleTipMessage message{StaleFork{&fork_point, &tip}, /*have_block=*/true};
    ...
    const auto [tip_hash, headers]{message.ReconstructHeaders()};
    BOOST_REQUIRE_EQUAL(headers.size(), 2U);
    BOOST_CHECK_EQUAL(headers.front().GetHash().ToString(), first.GetBlockHash().ToString());
    BOOST_CHECK_EQUAL(tip_hash.ToString(), tip.GetBlockHash().ToString());
}
