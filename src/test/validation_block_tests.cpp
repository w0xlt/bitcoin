...
BOOST_AUTO_TEST_CASE(processnewblock_check_outside_main)
{
    ...
    for (bool valid : {true, false}) {
        ...
        // Release main and join even when the watchdog fires (including the negative control).
        worker.join();
...
BOOST_AUTO_TEST_CASE(processnewblock_concurrent_invalidation)
{
    ...
    for (int round = 0; round < 4; ++round) {
        ...
        auto* parent_index{WITH_LOCK(cs_main, return chainman.m_blockman.LookupBlockIndex(parent->GetHash()))};
        ...
        // Either concurrent ordering is allowed. A later fresh child must see the invalid parent.
        bool new_block{true};
