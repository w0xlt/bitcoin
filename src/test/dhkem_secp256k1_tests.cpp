...
BOOST_AUTO_TEST_CASE(dhkem_secp256k1_chacha20poly1305_testvectors)
{
        ...
        // PDK test vector
        {
            ...
            /* exporter_secret */ "58732d8b645ec857be9847995103155b6c6276dda44c1e40b68f1463c03aabb7",
            {
                ...
                },
                {
                    /* pt    */ "4265617574792069732074727574682c20747275746820626561757479",
                    /* aad   */ "436f756e742d31",
                    /* nonce */ "7028e56372041618809a2564",
                    /* ct    */ "ba9d7537a21c7bcf729b97602dbb0eb5abf5a4ca2ac81f262f33ce6c508f82f5876853ef563849b81dd1dbbcea",
                },
    ...
    for (size_t i = 0; i < base_vecs.size(); ++i) {
        ...
        for (size_t j = 0; j < base_vecs[i].encryptions.size(); ++j) {

            const Encryption& enc = base_vecs[i].encryptions[j];
            ...
            std::cout << "---> seq_bytes: " << HexStr(seq_bytes) << std::endl;
            std::cout << "---> nonce:      " << HexStr(nonce) << std::endl;

            
            std::vector<uint8_t> mixed = mix_nonce(got_nonce, 0);

            std::cout << "---> mixed: " << HexStr(mixed) << std::endl;
    ...
    for (size_t i = 0; i < auth_vecs.size(); ++i) {
        ...
        BOOST_CHECK(dhkem_secp256k1::AuthEncap(shared_secret_enc, skEm, pkEm, pkRm, skSm));

        // 4. Use dhkem_secp256k1::AuthDecap with enc, skR, pkS to decapsulate and get shared_secret
        BOOST_CHECK(dhkem_secp256k1::AuthDecap(shared_secret_dec, pkEm, skRm, pkSm));
