...
namespace {
/** Parse a hex string in big-endian byte order into a uint256.
 *  BIP 346 test vectors use big-endian hex encoding for hashes. */
std::optional<uint256> uint256_from_hex_be(std::string_view str)
{
    if (uint256::size() * 2 != str.size() || !IsHex(str)) return std::nullopt;
    uint256 rv;
...
BOOST_FIXTURE_TEST_CASE(txhash_vectors, BasicTestingSetup)
{
    ...
    for (unsigned int g = 0; g < groups.size(); g++) {
        ...
        for (unsigned int v = 0; v < vectors.size(); v++) {
            ...
            auto expected_hash = uint256_from_hex_be(expected_hex);
