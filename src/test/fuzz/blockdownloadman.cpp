...
void initialize_blockdownloadman()
{
    ...
    for (int i = 0; i < 20; ++i) {
        auto block = PrepareBlock(g_setup->m_node, {.coinbase_output_script = P2WSH_OP_TRUE});
        MineBlock(g_setup->m_node, block);
        BLOCK_DATA.push_back(*block);
    }
...
FUZZ_TARGET(blockdownloadman, .init = initialize_blockdownloadman)
{
    ...
    LIMITED_WHILE(fuzzed_data_provider.ConsumeBool(), 500)
    {
        ...
        CallOneOf(
            ...
            [&] {
                ...
                LOCK(cs_main);
                bdm.ConnectedPeer(rand_peer, /*is_inbound=*/fuzzed_data_provider.ConsumeBool());
            ...
            [&] {
                ...
                LOCK(cs_main);
                bdm.FindNextBlocksToDownload(rand_peer, {fuzzed_data_provider.ConsumeBool(), fuzzed_data_provider.ConsumeBool()}, count, blocks, staller);
            ...
            [&] {
                ...
                LOCK(cs_main);
                bdm.TryDownloadingHistoricalBlocks(rand_peer, {fuzzed_data_provider.ConsumeBool(), fuzzed_data_provider.ConsumeBool()}, count, blocks, from_tip, target);
                check_scheduled_blocks(blocks, count);
            },
            ...
            [&] {
                if (!mirror.registered) return;
                const auto until = std::chrono::microseconds{fuzzed_data_provider.ConsumeIntegral<uint32_t>()};
                LOCK(cs_main);
                bdm.PauseDownload(rand_peer, until);
                Assert(bdm.GetDownloadPausedUntil(rand_peer) == until);
            },
            [&] { // Initialize compact reconstruction, including repeated announcements.
                if (!mirror.registered) return;
                const CBlock& block = BLOCK_DATA[fuzzed_data_provider.ConsumeIntegralInRange<size_t>(0, BLOCK_DATA.size() - 1)];
                CBlock candidate{block};
                if (fuzzed_data_provider.ConsumeBool()) {
                    // Preserve the header but corrupt its prefilled transaction,
                    // exercising reconstruction failure without changing the request hash.
                    CMutableTransaction tx{*candidate.vtx.front()};
                    tx.vout.front().nValue = fuzzed_data_provider.ConsumeIntegralInRange<int64_t>(0, MAX_MONEY);
                    candidate.vtx.front() = MakeTransactionRef(std::move(tx));
                }
                CBlockHeaderAndShortTxIDs compact{candidate, fuzzed_data_provider.ConsumeIntegral<uint64_t>()};
                LOCK(cs_main);
                const auto info = bdm.FindBlockInFlight(block.GetHash(), rand_peer);
                if (!info.requested_from_peer && info.already_in_flight >= MAX_CMPCTBLOCKS_INFLIGHT_PER_BLOCK) return;
                const auto* index = Assert(chainman.m_blockman.LookupBlockIndex(block.GetHash()));
                const auto result = bdm.InitCompactBlock(rand_peer, *index, compact, {}, *mempool);
                Assert(result.has_value() == !info.has_partial_block);
                Assert(bdm.FindBlockInFlight(block.GetHash(), rand_peer).has_partial_block);
            },
            [&] { // Complete reconstruction or retain its failed state for the caller.
                const auto& source = BLOCK_DATA[fuzzed_data_provider.ConsumeIntegralInRange<size_t>(0, BLOCK_DATA.size() - 1)];
                BlockTransactions transactions;
                transactions.blockhash = source.GetHash();
                if (fuzzed_data_provider.ConsumeBool()) transactions.txn.push_back(source.vtx.front());
                LOCK(cs_main);
                const auto info = bdm.FindBlockInFlight(transactions.blockhash, rand_peer);
                if (!info.has_partial_block || info.partial_block_failed) return;
                CBlock block;
                (void)bdm.FillBlock(rand_peer, transactions, block);
                Assert(bdm.FindBlockInFlight(transactions.blockhash, rand_peer).partial_block_failed);
            },
            [&] { // In-flight query consistency
                ...
                if (info.has_partial_block) Assert(info.requested_from_peer);
        ...
        for (NodeId peer = 0; peer < NUM_PEERS; ++peer) {
            ...
            const auto in_flight = bdm.GetNumBlocksInFlight(peer);
            total_in_flight += in_flight;
            downloading_from += in_flight > 0;
            sync_started += peers[peer].sync_started;
            Assert(bdm.GetRequestedBlockHeights(peer).size() == in_flight);
            Assert(bdm.GetFirstBlockInFlight(peer).has_value() == (in_flight > 0));
        }
    ...
    // Disconnect everybody and check that peer and request tracking is empty.
    LOCK(cs_main);
    for (NodeId peer = 0; peer < NUM_PEERS; ++peer) {
