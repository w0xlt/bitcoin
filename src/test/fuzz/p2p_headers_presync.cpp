...
void HeadersSyncSetup::SendMessage(FuzzedDataProvider& fuzzed_data_provider, CSerializedNetMsg&& msg)
{
    ...
    do {
        ...
        completion_pending = m_node.peerman->WaitForBlockProcessing();
    } while (completion_pending);
}
...
FUZZ_TARGET(p2p_headers_presync, .init = initialize)
{
    ...
    g_testing_setup->m_node.peerman->StartBlockProcessing();
    ...
    g_testing_setup->m_node.peerman->StopBlockProcessing();
