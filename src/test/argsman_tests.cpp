...
#ifdef WIN32
    const char* argv_windows_slash_test[] = {"-ignored", "f", "/D=windows"};
    BOOST_CHECK(testArgs.ParseParameters(3, argv_windows_slash_test, error));
    BOOST_CHECK(testArgs.IsArgSet("-d"));
    BOOST_CHECK_EQUAL(testArgs.GetArg("-d", ""), "windows");

    const char* argv_windows_case_test[] = {"-ignored", "f", "-D=windows"};
    BOOST_CHECK(testArgs.ParseParameters(3, argv_windows_case_test, error));
    BOOST_CHECK(testArgs.IsArgSet("-d"));
    BOOST_CHECK_EQUAL(testArgs.GetArg("-d", ""), "windows");
#endif
}
