...
ReadStatus PartiallyDownloadedChunkBlock::InitData(const CBlockHeaderAndLengthShortTxIDs& comprblock, const std::vector<CTransactionRef>& extra_txn) {
    ...
    *decoded_block = header;
    ...
    if (index_offsets.size()) {
        ...
        // Use the calculated size, but ensure it's at least large enough for safety
        size_t actual_block_size = std::max(calculated_size, size_t(MAX_BLOCK_SERIALIZED_SIZE));
        
        size_t codedBlockSize = DIV_CEIL(actual_block_size, FEC_CHUNK_SIZE) * FEC_CHUNK_SIZE;
