...
static void PushMixedInputFields(UniValue& entry, const WalletTxHistoryAccounting& accounting)
{
    ...
    entry.pushKV("fee_known", accounting.fee.has_value());
}

static void PushMixedInputSummary(UniValue& entry, const WalletTxHistoryAccounting& accounting)
{
    entry.pushKV("category", "mixed");
    entry.pushKV("amount", ValueFromAmount(accounting.net));
...
static void ListTransactions(const CWallet& wallet, const CWalletTx& wtx, int nMinDepth, bool fLong,
                             ...
                             bool include_mixed_received = false)
    EXCLUSIVE_LOCKS_REQUIRED(wallet.cs_wallet)
{
    CAmount nFee;
    ...
    // Received
    const bool include_received_entries{!needs_mixed_input_summary || include_mixed_received || filter_label.has_value()};
    if (include_received_entries && listReceived.size() > 0 && wallet.GetTxDepthInMainChain(wtx) >= nMinDepth) {
...
static std::vector<RPCResult> TransactionDescriptionString()
{
    return{{RPCResult::Type::NUM, "confirmations", "The number of confirmations for the transaction. Negative confirmations means the\n"
           ...
           {RPCResult::Type::STR_AMOUNT, "wallet_credit", /*optional=*/true, "Only present if involves_mixed_inputs is true. Total value of wallet-owned outputs received by this transaction."},
           {RPCResult::Type::BOOL, "fee_known", /*optional=*/true, "Only present if involves_mixed_inputs is true. Whether the wallet knows the fee attributable to this entry."},
           };
}
...
RPCMethod listtransactions()
{
    return RPCMethod{
        ...
        "If a label name is provided, this will return only incoming transactions paying to addresses with the specified label.\n"
                ...
                "all non-wallet inputs have known zero value. Otherwise, it is reported as a mixed entry with the wallet's \n"
                "net amount, because the wallet cannot attribute foreign outputs or fee share without additional metadata.\n",
                {
                    ...
                    RPCResult::Type::ARR, "", "",
                    {
                        {RPCResult::Type::OBJ, "", "", Cat(Cat<std::vector<RPCResult>>(
                        {
                            ...
                            {RPCResult::Type::STR, "category", "The transaction category.\n"
                                ...
                                "\"mixed\"                 Mixed-input transactions where sent outputs or fee share cannot be attributed. The amount is the wallet net amount.\n"
                                "\"receive\"               Non-coinbase transactions received.\n"
                                "\"generate\"              Coinbase transactions received with more than 100 confirmations.\n"
                                "\"immature\"              Coinbase transactions received with 100 or fewer confirmations.\n"
                                "\"orphan\"                Orphaned coinbase transactions received."},
                            {RPCResult::Type::STR_AMOUNT, "amount", "The amount in " + CURRENCY_UNIT + ". This is negative for the 'send' category, positive\n"
                                "for receive categories, and the wallet net amount for the 'mixed' category."},
        ...
        [](const RPCMethod& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    entry.pushKV("amount", ValueFromAmount(accounting.net + accounting.fee.value_or(0)));
    ...
    ListTransactions(*pwallet, wtx, 0, false, details, /*filter_label=*/std::nullopt, /*include_change=*/false, /*include_mixed_received=*/true);
