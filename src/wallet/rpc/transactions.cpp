...
// Non-wallet inputs may pay for some outputs or fees. The wallet cannot tell
// which non-wallet outputs are payments made by this wallet or how much of the
// fee it paid. It only knows the total value of wallet-owned inputs. Report
// that value with its sign reversed as one mixed-input send summary. Omit
// address and vout because the summary is not tied to one output. Omit fee
// because the wallet's fee share is unknown.
static void PushMixedInputSendSummary(UniValue& entry, const WalletTxHistoryAccounting& accounting)
{
    const CAmount total_wallet_input_value{accounting.debit};
    entry.pushKV("category", "send");
    entry.pushKV("amount", ValueFromAmount(-total_wallet_input_value));
...
RPCMethod listtransactions()
{
    return RPCMethod{
        ...
        "If a label name is provided, this will return only incoming transactions paying to addresses with the specified label.\n"
                ...
                "the wallet's fee share is unknown. Wallet-owned outputs are reported as separate receive entries, so the transaction's \n"
                    ...
                    RPCResult::Type::ARR, "", "",
                    {
                        {RPCResult::Type::OBJ, "", "", Cat(Cat<std::vector<RPCResult>>(
                        {
                            ...
                            {RPCResult::Type::STR, "category", "The transaction category.\n"
                                "\"send\"                  Transactions sent. For mixed-input transactions where non-wallet inputs may pay for outputs or fees, one send summary reports the total value of wallet-owned inputs with its sign reversed.\n"
                                "\"receive\"               Non-coinbase transactions received.\n"
                                "\"generate\"              Coinbase transactions received with more than 100 confirmations.\n"
                                "\"immature\"              Coinbase transactions received with 100 or fewer confirmations.\n"
                                "\"orphan\"                Orphaned coinbase transactions received."},
                            {RPCResult::Type::STR_AMOUNT, "amount", "The amount in " + CURRENCY_UNIT + ". This is non-positive for the 'send' category and non-negative for\n"
                                "receive categories. For a mixed-input send summary, this is the total value of wallet-owned inputs with its sign reversed."},
                            {RPCResult::Type::STR, "label", /*optional=*/true, "A comment for the address/transaction, if any"},
                            {RPCResult::Type::NUM, "vout", /*optional=*/true, "the vout value"},
                            {RPCResult::Type::STR_AMOUNT, "fee", /*optional=*/true, "The transaction fee paid by this wallet in " + CURRENCY_UNIT + ", shown as zero or a negative amount. Only included in\n"
                                 "'send' entries; omitted if a non-wallet input may have helped pay the fee."},
                        },
    ...
    return RPCMethod{
        ...
        "Get detailed information about in-wallet transaction <txid>\n",
                ...
                RPCResult{
                    RPCResult::Type::OBJ, "", "", Cat(Cat<std::vector<RPCResult>>(
                    {
                        {RPCResult::Type::STR_AMOUNT, "amount", "The transaction amount in " + CURRENCY_UNIT + ". A known wallet fee is reported separately."},
                        {RPCResult::Type::STR_AMOUNT, "fee", /*optional=*/true, "The fee paid by this wallet in " + CURRENCY_UNIT + ", shown as zero or a negative amount. Only included when a wallet-owned input is spent; omitted if a non-wallet input may have helped pay the fee."},
                    },
                    TransactionDescriptionString()),
                    {
        ...
        [](const RPCMethod& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    const CAmount balance_change{accounting.credit - accounting.debit};
    const CAmount fee_paid{accounting.fee.value_or(0)};
    // A known fee is reported separately with the opposite sign, so add it
    // back to the balance change to exclude it from amount. If the wallet's
    // fee share is unknown, no fee is separated and amount is the balance
    // change.
    entry.pushKV("amount", ValueFromAmount(balance_change + fee_paid));
    if (accounting.fee.has_value()) {
        entry.pushKV("fee", ValueFromAmount(-fee_paid));
    }
