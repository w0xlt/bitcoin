...
RPCHelpMan exposesecret()
{
    return RPCHelpMan{
        "exposesecret",
        "Return the BIP32 wallet seed in codex32 format for descriptor wallets.\n"
        ...
        RPCResult{
            ...
            {
                {RPCResult::Type::STR, "codex32", "(string) The seed encoded in codex32 format. If the wallet has multiple seeds, an array of codex32 strings is returned."}
            }
        },
        RPCExamples{
            HelpExampleCli("exposesecret", "") +
            HelpExampleCli("exposesecret", "\"test\"") +
            HelpExampleCli("exposesecret", "\"test\" \"ms\"")
        },
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    std::shared_ptr<CWallet> const pwallet = GetWalletForJSONRPCRequest(request);
    ...
    if (!wallet->IsWalletFlagSet(WALLET_FLAG_DESCRIPTORS)) {
        throw JSONRPCError(RPC_WALLET_ERROR, "exposesecret is only available for descriptor wallets");
    }
...
RPCHelpMan recoverwalletfromseed()
{
    return RPCHelpMan{
        ...
        "Recover and load an unencrypted wallet from seed.\n"
        "Call encryptwallet afterwards if the recovered wallet should be encrypted.\n"
        "\nThe rescan is significantly faster if block filters are available"
        "\n(using startup option \"-blockfilterindex=1\").\n",
        {
            {"wallet_name", RPCArg::Type::STR, RPCArg::Optional::NO, "The name that will be applied to the restored wallet"},
        ...
        RPCExamples{
            ...
            + HelpExampleCliNamed("recoverwalletfromseed", {{"wallet_name", "testwallet"}, {"codex32_seed", "wr10w2xxsrl466a09f9fcjuncaxsafeq0w9dlmpxmywr4w6sganxwvv9cn4nsdkx0v3ca6wewv"}, {"load_on_startup", true}})
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    const std::string codex32_seed = request.params[1].get_str();
    std::string hrp = "wr";

    std::optional<bool> load_on_start = request.params[2].isNull() ? std::nullopt : std::optional<bool>(request.params[2].get_bool());
    bool rescan = request.params[3].isNull() ? true : request.params[3].get_bool();
    int start_height = request.params[4].isNull() ? 0 : request.params[4].getInt<int>();

    // Decode the codex32 secret
    std::string error_msg;
    std::optional<Codex32> codex = codex32_decode(hrp, codex32_seed, error_msg);
    if (!codex.has_value()) {
        throw JSONRPCError(RPC_INVALID_PARAMETER, strprintf("Invalid codex32_seed: %s", error_msg));
    }
    if (codex->type != Codex32Encoding::SECRET) {
        throw JSONRPCError(RPC_INVALID_PARAMETER, "codex32_seed must represent a full secret (not a share)");
    }

    const std::vector<unsigned char>& seed_bytes = codex->payload;
    if (seed_bytes.size() < 16 || seed_bytes.size() > 32) {
        throw JSONRPCError(RPC_INVALID_PARAMETER, "codex32_seed decoded seed must be between 128 and 256 bits (16-32 bytes)");
    }
    ...
    if (!key.IsValid()) {
        throw JSONRPCError(RPC_INVALID_PARAMETER, "codex32_seed is invalid or produces an out-of-range seed");
    }
