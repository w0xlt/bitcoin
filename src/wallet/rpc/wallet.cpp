...
//! BIP-93 reference HRP. See note in scriptpubkeyman.cpp.
...
static RPCMethod addhdkey()
{
    return RPCMethod{
        ...
        RPCExamples{
            HelpExampleCli("addhdkey", "\"ms10f2tvsugydzy9ggegddt5tyamkawpve4ukxvvdntmhwvr2f9se3sf6r54sr3edm5vrhz3t2\"")
        },
        [](const RPCMethod& self, const JSONRPCRequest& request) -> UniValue
        {
            std::shared_ptr<CWallet> const pwallet = GetWalletForJSONRPCRequest(request);
            ...
            if (codex32->payload.size() < CODEX32_MIN_SEED_BYTES || codex32->payload.size() > CODEX32_MAX_SEED_BYTES) {
                throw JSONRPCError(RPC_INVALID_PARAMETER,
                    strprintf("Expected a %u to %u byte seed, got %u bytes",
                              CODEX32_MIN_SEED_BYTES, CODEX32_MAX_SEED_BYTES, codex32->payload.size()));
            }
