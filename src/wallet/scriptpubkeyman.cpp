...
namespace {
//! BIP-93 reference HRP. Using "ms" keeps backups interoperable with external
//! codex32 tooling (verifiers, share generators, hardware-wallet importers).
static constexpr std::string_view CODEX32_WALLET_HRP{"ms"};
...
bool Codex32SecretMatchesXpub(const CKeyingMaterial& secret, const CExtPubKey& xpub)
{
    std::string error;
    const auto codex32 = Codex32Decode(std::string{CODEX32_WALLET_HRP}, std::string{secret.begin(), secret.end()}, error);
    if (!codex32 || codex32->type != Codex32Encoding::SECRET ||
        codex32->payload.size() < CODEX32_MIN_SEED_BYTES || codex32->payload.size() > CODEX32_MAX_SEED_BYTES) {
        return false;
    }
...
bool DescriptorScriptPubKeyMan::CheckDecryptionKey(const CKeyingMaterial& master_key)
{
    ...
    for (const auto& [xpub, crypted] : m_crypted_codex32_secrets) {
        ...
        if (!DecryptSecret(master_key, crypted, DescriptorCodex32IV(desc_id, xpub), secret) ||
            !Codex32SecretMatchesXpub(secret, xpub)) {
            return false;
        }
    }
...
bool DescriptorScriptPubKeyMan::Encrypt(const CKeyingMaterial& master_key, WalletBatch* batch)
{
    ...
    const uint256 desc_id = GetID();
    const bool had_codex32_secrets = !m_codex32_secrets.empty();
    for (const auto& [xpub, secret] : m_codex32_secrets) {
        ...
        if (!batch->WriteCryptedDescriptorCodex32(desc_id, xpub, crypted)) {
            return false;
        }
        m_crypted_codex32_secrets[xpub] = crypted;
    }
    m_codex32_secrets.clear();
    if (had_codex32_secrets && !m_storage.IsWalletFlagSet(WALLET_FLAG_CODEX32_SECRETS)) {
...
bool DescriptorScriptPubKeyMan::SetCodex32Secret(WalletBatch& batch, const CExtPubKey& xpub, const std::string& codex32)
{
    ...
    const CKeyingMaterial secret{codex32.begin(), codex32.end()};
    if (!batch.WriteDescriptorCodex32(GetID(), xpub, secret)) {
        return false;
    }
