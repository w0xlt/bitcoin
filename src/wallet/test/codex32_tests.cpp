...
BOOST_AUTO_TEST_CASE(codex32_encode_invalid_inputs)
{
    ...
    BOOST_CHECK(Codex32SecretEncode("", "leet", 0, seed, error).empty());
    BOOST_CHECK_EQUAL(error, "Invalid hrp: must not be empty");

    error.clear();
    BOOST_CHECK(Codex32SecretEncode("Ms", "leet", 0, seed, error).empty());
    BOOST_CHECK_EQUAL(error, "Invalid hrp: must be printable ASCII and lower-case");
...
{
    std::string error;
    const auto parts = Codex32Decode("", "ms10testsxxxxxxxxxxxxxxxxxxxxxxxxxywvfucx7rv8mk8", error);
...
{
    std::string error;
    const auto parts = Codex32Decode("ms", "MS12NAMES6XQGUZTTXKEQNJSJZV4JV3NZ552F9CS6V8Q9Q98", error);
...
{
    const std::string long_codex = "MS100C8VSM32ZXFGUHPCHTLUPZRY9X8GF2TVDW0S3JN54KHCE6MUA7LQPZYGSFJD6AN074RXVCEMLH8WU3TK925ACDEFGHJKLMNPQRSTUVWXY06GCT4AX9XTMG9J4EP";
...
BOOST_AUTO_TEST_CASE(codex32_rejects_nonzero_payload_padding)
{
    const std::vector<std::string> addresses = {
        "wr10f2tvsugydzy9ggegddt5tyamkawpve4ukxvvdntmhwvr2f9se3sf6r543xardnvslx95t3",
    ...
    for (const auto& address : addresses) {
        ...
        BOOST_CHECK(!parts.has_value());
        BOOST_CHECK_EQUAL(error, "Invalid payload!");
    }
}
...
{
    std::string error;
    const auto parts = Codex32Decode("", "ms10fauxxxxxxxxxxxxxxxxxxxxxxxxxxxy5ap2tjg5tqhcg", error);
...
{
    std::string error;
    const auto parts = Codex32Decode("", "ms1fauxxsxxxxxxxxxxxxxxxxxxxxxxxxxy9qk27vvylrks2", error);
...
BOOST_AUTO_TEST_CASE(codex32_roundtrip_property)
{
    // Property test: for random seeds of every BIP-93-supported length and
    // all valid (hrp, id, threshold) combinations, encoding then decoding
    // must reproduce every field bit-for-bit.
    constexpr size_t kIterationsPerLen = 64;
    const std::vector<size_t> seed_lens{16, 18, 20, 24, 28, 32, 40, 48, 56, 64};
    const std::vector<std::string> hrps{"ms", "cl", "wr", "x", "longhrp"};
    const std::vector<uint8_t> thresholds{0, 2, 3, 4, 5, 6, 7, 8, 9};

    for (size_t seed_len : seed_lens) {
        for (size_t i = 0; i < kIterationsPerLen; ++i) {
            const std::string& hrp = hrps[m_rng.randrange(hrps.size())];
            const uint8_t threshold = thresholds[m_rng.randrange(thresholds.size())];

            std::string id;
            id.reserve(4);
            for (int j = 0; j < 4; ++j) {
                id += bech32::CHARSET[m_rng.randrange<uint8_t>(32)];
            }

            const std::vector<uint8_t> seed = m_rng.randbytes<uint8_t>(seed_len);

            std::string encode_error;
            const std::string encoded = Codex32SecretEncode(hrp, id, threshold, seed, encode_error);
            BOOST_REQUIRE_MESSAGE(encode_error.empty(),
                "encode failed: " << encode_error << " (hrp=" << hrp
                << " id=" << id << " threshold=" << int{threshold}
                << " seed_len=" << seed_len << ")");
            BOOST_REQUIRE(!encoded.empty());

            std::string decode_error;
            const auto parts = Codex32Decode(hrp, encoded, decode_error);
            BOOST_REQUIRE_MESSAGE(parts.has_value(),
                "decode failed: " << decode_error << " (encoded=" << encoded << ")");
            BOOST_CHECK(decode_error.empty());

            BOOST_CHECK_EQUAL(parts->hrp, hrp);
            BOOST_CHECK_EQUAL(int{parts->threshold}, int{threshold});
            BOOST_CHECK_EQUAL(std::string(parts->id.data()), id);
            BOOST_CHECK_EQUAL(parts->share_idx, 's');
            BOOST_CHECK(parts->type == Codex32Encoding::SECRET);
            BOOST_CHECK_EQUAL(HexStr(parts->payload), HexStr(seed));

            // Re-encoding the decoded parts must yield the original string.
            std::string reencode_error;
            const std::string reencoded = Codex32SecretEncode(parts->hrp, std::string(parts->id.data()),
                                                              parts->threshold, parts->payload, reencode_error);
            BOOST_CHECK(reencode_error.empty());
