...
void CWallet::RecursiveUpdateTxState(WalletBatch* batch, const Txid& tx_hash, const TryUpdatingStateFn& try_updating_state) {
    ...
    while (!todo.empty()) {
        ...
        if (update_state != TxUpdate::UNCHANGED) {
            wtx.MarkDirty();
            if (batch) WriteTransation(*batch, wtx);
    ...
    for (const auto& [_pos, wtx] : wtxOrdered) {
        ...
        if (data.watchonly_wallet) {
            ...
            if (data.watchonly_wallet->IsMine(*wtx->tx) || data.watchonly_wallet->IsFromMe(*wtx->tx)) {
                ...
                WriteTransation(*watchonly_batch, data.watchonly_wallet->mapWallet.at(hash));
