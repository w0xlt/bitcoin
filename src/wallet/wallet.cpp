 ...
 */
static unsigned int ComputeTimeSmart(interfaces::Chain& chain,
                                     const CWalletTx& wtx,
                                     const std::multimap<int64_t, CWalletTx*>& tx_ordered,
    ...
    if (block_hash) {
        ...
        if (chain.findBlock(*block_hash, FoundBlock().time(blocktime).maxTime(block_max_time))) {
            if (rescanning_old_block) {
                ...
                for (auto it = tx_ordered.rbegin(); it != tx_ordered.rend(); ++it) {
                    CWalletTx* const pwtx = it->second;
                    if (pwtx == &wtx) {
                        continue;
                    }
            ...
            LogInfo("%s: found %s in block %s not in index\n", __func__, wtx.GetHash().ToString(), block_hash->ToString());
        }
    }
    return nTimeSmart;
}
...
CWalletTx* CWallet::AddToWallet(CTransactionRef tx, const TxState& state, const UpdateWalletTxFn& update_wtx, bool rescanning_old_block)
{
    ...
    if (fInsertedNew) {
        ...
        wtx.nTimeSmart = ComputeTimeSmart(chain(), wtx, wtxOrdered, rescanning_old_block);
        AddToSpends(wtx);
