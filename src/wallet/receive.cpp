...
// Transaction inputs identify previous outputs but do not contain their values.
// For a non-wallet input, the wallet can obtain the value only if it also stores
// the parent transaction. Return false when the parent or output is unavailable
// so mixed-input history accounting remains conservative.
