...
private:
    ...
    bool AddSilentScriptKeyMan(const CTransaction& tx, const SyncTxState& state, bool rescanning_old_block, const std::vector<CKey>& rawTrKeys) EXCLUSIVE_LOCKS_REQUIRED(cs_wallet);

    /** Detect if a transaction is a silent payment that belongs to wallet and in that case add it **/
    bool VerifySilentPayment(const CTransaction& tx, std::vector<CKey>& rawTrKeys) EXCLUSIVE_LOCKS_REQUIRED(cs_wallet);
...
public:
    ...
    util::Result<CTxDestination> GetNewDestination(const OutputType type, const std::string label, const bool addToAddressBook = true);
