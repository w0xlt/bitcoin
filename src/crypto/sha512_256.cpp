...
namespace
{
/// Internal SHA-512 implementation (shared compression function).
namespace sha512
{
...
/** Initialize SHA-512/256 state (FIPS 180-4 specified IVs). */
...
CSHA512_256::CSHA512_256()
{
    sha512::Initialize(s);
}

CSHA512_256& CSHA512_256::Write(const unsigned char* data, size_t len)
{
...
void CSHA512_256::Finalize(unsigned char hash[OUTPUT_SIZE])
{
    ...
    Write(sizedesc, 16);
    // Output only the first 256 bits (32 bytes)
    WriteBE64(hash, s[0]);
