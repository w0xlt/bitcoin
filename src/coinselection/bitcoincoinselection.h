 ...
 * - Waste metric now uses Bitcoin Core's SelectionResult::RecalculateWaste()
 *   instead of a simplified custom calculation.
 * - Selection functions now require additional parameters (min_viable_change,
 *   cost_of_change, change_fee) for accurate waste calculation.
 * - btccs_calculate_cost_of_change now requires explicit discard_feerate.
 * - Added btccs_calculate_fee helper function.
 */
