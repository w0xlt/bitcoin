    ...
    while (true) {
        ...
        do {
            ...
            if (block.is_header_processing) {
                ...
                ReadStatus decode_status = block.ProvideHeaderData(header, peer_manager->GetExtraTxnForCompact());
...
ReadStatus PartialBlockData::ProvideHeaderData(const CBlockHeaderAndLengthShortTxIDs& header, std::vector<std::pair<Wtxid, CTransactionRef>>& extra_txn) {
    assert(in_header);
    in_header = false;
    initialized = false;
