...
private:
    ...
    bool ProcessOptionKey(
        ...
        std::string& error,
        const bool found_after_non_option = false) EXCLUSIVE_LOCKS_REQUIRED(cs_args);
    ...
    bool HandleCommandOption(const char* const arg, std::string& error) EXCLUSIVE_LOCKS_REQUIRED(cs_args);
