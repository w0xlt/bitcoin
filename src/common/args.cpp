...
fs::path ArgsManager::GetDataDirBase() const { LOCK(cs_args); return GetDataDir(false); }
fs::path ArgsManager::GetDataDirNet() const { LOCK(cs_args); return GetDataDir(true); }
