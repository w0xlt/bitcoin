...
#include <fiber/udpnet.h>
#include <fiber/udprelay.h>
...
bool InitializeUDPConnections() {
    ...
    udp_write_threads.emplace_back(util::TraceThread<void(*)()>, 
    ...
    if (gArgs.IsArgSet("-fecreaddevice")) {
        ...
        udp_local_read_thread = new std::thread(util::TraceThread<void(*)()>, 
                                                "udpreadlocal", ThreadRunLocalReadEventLoop);
    }

    BlockRecvInit();

    udp_read_thread = new std::thread(util::TraceThread<void(*)()>, 
...
static void StartLocalBackfillThread() {
    ...
    std::thread(util::TraceThread<void(*)()>,
                "udpbackfill", []() {
