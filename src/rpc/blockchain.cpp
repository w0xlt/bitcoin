...
struct SilentScanner {
	...
	std::map<COutPoint, Coin> scan(const COutPoint key, const Coin coin)
	{
		...
		for(auto& [silent_recipient, range]: recipient_range) {
            ...
            // const auto& [sum_of_all_input_pubkeys, outpoint_hash] = g_silentpaymentindex->FindSilentPaymentByTransactionId(key.hash);
    ...
    void GetSilentData(const uint256& txid)
    {
        ...
        if (!(UndoReadFromDisk(blockUndo, pindex))) {
            throw JSONRPCError(RPC_MISC_ERROR, "Undo data not found for block " + hashBlock.ToString());
        }
