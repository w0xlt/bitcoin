    ...
    const std::pair<int64_t, int64_t>& range)
{
    ...
    auto sum_of_all_input_pubkeys{XOnlyPubKey()};

    if(!g_silentpaymentindex->FindSilentPayment(txhash, sum_of_all_input_pubkeys)) {
        return false;
    }

    silentpayment::Recipient silent_sender{privKey, sum_of_all_input_pubkeys};

    for (int64_t identifier = range.first; identifier <= range.second; identifier++) {
        CKey silKey{silent_sender.Tweak((int32_t) identifier)};
