...
// === Replace both helpers in the RPC module ===
...
static RPCHelpMan fetchohttpkeys()
{
    return RPCHelpMan{
            ...
            "Fetch OHTTP KeyConfig from the Payjoin Directory through an OHTTP Relay (HTTP CONNECT bootstrap).\n",
            {
                {"ohttp_relay", RPCArg::Type::STR, RPCArg::Default{"https://pj.bobspacebkk.com"}, "OHHTP Relay URL"},
                {"payjoin_directory", RPCArg::Type::STR, RPCArg::Default{"https://payjo.in"}, "Payjoin Directory origin"},
            ...
            RPCResult{
                RPCResult::Type::OBJ, "", "",
                {
                    {RPCResult::Type::STR_HEX, "keys_raw", "Raw application/ohttp-keys body"},
                    {RPCResult::Type::ARR, "parsed", "Parsed entries",
                        {{RPCResult::Type::STR_HEX, "", "KeyConfig object"}},},
                }
            },
            RPCExamples{
                HelpExampleCli("fetchohttpkeys", "\"https://pj.bobspacebkk.com\" \"https://payjo.in\"") + 
            ...
            [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    std::cout << "Fetching OHTTP keys from directory " << directory << " via relay " << proxy << std::endl;

    // RFC 9540 / Payjoin docs: fetch at /.well-known/ohttp-gateway with Accept: application/ohttp-keys. :contentReference[oaicite:18]{index=18}
    std::string url = directory;
    if (!url.empty() && url.back() == '/') url.pop_back();
    url += "/.well-known/ohttp-gateway";

    std::cout << "Fetching OHTTP keys from URL " << url << std::endl;

    std::vector<uint8_t> body = HttpGetViaProxy(proxy, url);

    std::cout << "Fetched " << body.size() << " bytes" << std::endl;

    std::cout << "OHTTP keys body: " << HexStr(body) << std::endl;

    ohttp::OhttpKeys keys = ohttp::ParseOhttpKeys(body);

    std::cout << "Parsed " << keys.configs.size() << " key configs" << std::endl;

    UniValue arr(UniValue::VARR);
    for (const auto& k : keys.configs) arr.push_back(KeyConfigToJSON(k));

    UniValue result(UniValue::VOBJ);
    result.pushKV("keys_raw", EncodeBase64(body));
    result.pushKV("parsed", arr);
    return result;
},
    };
