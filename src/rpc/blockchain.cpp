...
static void CreateDatabaseSchema(sqlite3* db) {
    ...
    // Partial unique index to ensure only one active block per height
    ExecSqlOrThrow(db, "CREATE UNIQUE INDEX IF NOT EXISTS idx_blocks_active_height ON blocks(height) WHERE is_active = 1;");
                            ...
                            const CTxMemPool* const mempool)
{
    ...
    sqlite3_bind_int64(insert_block_stmt.get(), 3, static_cast<sqlite3_int64>(block.nTime));
    sqlite3_bind_int(insert_block_stmt.get(), 4, 1);                                          // is_active (true for initial scan)
    ...
    for (size_t tx_idx = 0; tx_idx < block.vtx.size(); ++tx_idx) {
        ...
        sqlite3_bind_int64(insert_tx_or_ignore_stmt.get(), 2, static_cast<sqlite3_int64>(tx->version));
        sqlite3_bind_int64(insert_tx_or_ignore_stmt.get(), 3, static_cast<sqlite3_int64>(tx->nLockTime));
        ...
        for (size_t vin_idx = 0; vin_idx < tx->vin.size(); ++vin_idx) {
            ...
            sqlite3_bind_int64(insert_input_stmt.get(), 6, static_cast<sqlite3_int64>(txin.nSequence));
        ...
        for (size_t vout_idx = 0; vout_idx < tx->vout.size(); ++vout_idx) {
            ...
            if (ExtractDestination(txout.scriptPubKey, destination)) {
                ...
                std::optional<OutputType> output_type = OutputTypeFromDestination(destination);

                if (output_type) {
                    std::string formatted_output_type = FormatOutputType(*output_type);
                ...
                [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    try {
        ...
        SQLiteStatement insert_block_stmt(db, "INSERT INTO blocks (height, block_hash, timestamp, is_active) VALUES (?, ?, ?, ?)");
