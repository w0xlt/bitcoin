...
public:
    ...
    bool submitSolution(uint32_t version, uint32_t timestamp, uint32_t nonce, CTransactionRef coinbase, bool precious) override
    {
        AddMerkleRootAndCoinbase(m_block_template->block, std::move(coinbase), version, timestamp, nonce);
        std::string reason;
        std::string debug;
        return SubmitBlock(chainman(), std::make_shared<const CBlock>(m_block_template->block), /*new_block=*/nullptr, precious, reason, debug);
    }
...
public:
    ...
    bool submitBlock(const CBlock& block_in, bool precious, std::string& reason, std::string& debug) override
    {
        ...
        bool new_block;
        const bool accepted = SubmitBlock(chainman(), block, &new_block, precious, reason, debug);
        // ProcessNewBlock() can accept and store a block before it is checked
        // for validity. By default, duplicate submissions and inconclusive
        // validation set reason. With precious=true, an already-known block can
        // still succeed if it is validated/connected. Report success only when
        // the block was accepted and no reason was set.
