...
void BlockDownloadManager::ConnectedPeer(NodeId nodeid, bool is_inbound)
{
    m_impl->ConnectedPeer(nodeid, is_inbound);
}
...
{
    return m_impl->BlockRequested(nodeid, block);
}
...
void BlockDownloadManager::FindNextBlocksToDownload(NodeId nodeid, const BlockDownloadPeerInfo& info, unsigned int count,
                                                    std::vector<const CBlockIndex*>& vBlocks,
                                                    NodeId& nodeStaller)
{
    m_impl->FindNextBlocksToDownload(nodeid, info, count, vBlocks, nodeStaller);
}

void BlockDownloadManager::TryDownloadingHistoricalBlocks(NodeId nodeid, const BlockDownloadPeerInfo& info, unsigned int count,
                                                          std::vector<const CBlockIndex*>& vBlocks,
                                                          const CBlockIndex* from_tip,
                                                          const CBlockIndex* target_block)
{
    m_impl->TryDownloadingHistoricalBlocks(nodeid, info, count, vBlocks, from_tip, target_block);
}
...
size_t BlockDownloadManager::GetNumBlocksInFlight(NodeId nodeid) const
{
    return Assert(m_impl->GetPeerState(nodeid))->vBlocksInFlight.size();
}

std::vector<int> BlockDownloadManager::GetRequestedBlockHeights(NodeId nodeid) const
{
    std::vector<int> heights;
    for (const auto& queued : Assert(m_impl->GetPeerState(nodeid))->vBlocksInFlight) {
        heights.push_back(queued.pindex->nHeight);
    }
    return heights;
}

std::optional<uint256> BlockDownloadManager::GetFirstBlockInFlight(NodeId nodeid) const
{
    const auto& blocks = Assert(m_impl->GetPeerState(nodeid))->vBlocksInFlight;
    if (blocks.empty()) return std::nullopt;
    return blocks.front().pindex->GetBlockHash();
}

void BlockDownloadManager::PauseDownload(NodeId nodeid, std::chrono::microseconds until)
{
    Assert(m_impl->GetPeerState(nodeid))->m_block_download_paused_until = until;
}

std::chrono::microseconds BlockDownloadManager::GetDownloadPausedUntil(NodeId nodeid) const
{
    return Assert(m_impl->GetPeerState(nodeid))->m_block_download_paused_until;
}
    ...
    for (auto it = range.first; it != range.second; ++it) {
        if (it->second.first == peer_id) {
            ...
            if (it->second.second->partialBlock) {
                ...
                result.partial_block_failed = it->second.second->partialBlock->header.IsNull();
            }
            break;
        }
    }
...
void BlockDownloadManagerImpl::ConnectedPeer(NodeId nodeid, bool is_inbound)
{
    const auto [it, inserted] = m_peer_info.try_emplace(nodeid, is_inbound);
    Assume(inserted);
}
...
bool BlockDownloadManagerImpl::IsBlockRequestedFromOutbound(const uint256& hash) const
{
    for (auto range = mapBlocksInFlight.equal_range(hash); range.first != range.second; range.first++) {
        ...
        if (state && !state->m_is_inbound) return true;
    }
    return false;
}
...
bool BlockDownloadManagerImpl::BlockRequested(NodeId nodeid, const CBlockIndex& block)
{
    ...
    if (GetBlockRequest(nodeid, hash)) return false;

    const auto it = state->vBlocksInFlight.insert(state->vBlocksInFlight.end(), {&block, nullptr});
...
BlockDownloadManagerImpl::QueuedBlock* BlockDownloadManagerImpl::GetBlockRequest(NodeId nodeid, const uint256& hash)
{
    for (auto range = mapBlocksInFlight.equal_range(hash); range.first != range.second; ++range.first) {
        if (range.first->second.first == nodeid) return &*range.first->second.second;
    }
    return nullptr;
}

std::optional<BlockDownloadManager::CompactBlockResult> BlockDownloadManager::InitCompactBlock(
    NodeId nodeid, const CBlockIndex& block, const CBlockHeaderAndShortTxIDs& cmpctblock,
    const std::vector<std::pair<Wtxid, CTransactionRef>>& extra_txns, CTxMemPool& mempool)
{
    m_impl->BlockRequested(nodeid, block);
    auto& queued = *Assert(m_impl->GetBlockRequest(nodeid, block.GetBlockHash()));
    if (queued.partialBlock) return std::nullopt;
    queued.partialBlock = std::make_unique<PartiallyDownloadedBlock>(&mempool);
    CompactBlockResult result{queued.partialBlock->InitData(cmpctblock, extra_txns), {}};
    if (result.status == READ_STATUS_OK) {
        for (size_t i = 0; i < cmpctblock.BlockTxCount(); ++i) {
            if (!queued.partialBlock->IsTxAvailable(i)) result.missing_transactions.push_back(i);
        }
    }
    return result;
}

ReadStatus BlockDownloadManager::FillBlock(NodeId nodeid, const BlockTransactions& transactions, CBlock& block)
{
    auto& partial = *Assert(Assert(m_impl->GetBlockRequest(nodeid, transactions.blockhash))->partialBlock);
    Assume(!partial.header.IsNull());
    const CBlockIndex* prev_block{Assume(m_impl->m_opts.m_chainman.m_blockman.LookupBlockIndex(partial.header.hashPrevBlock))};
    return partial.FillBlock(block, transactions.txn,
        /*segwit_active=*/DeploymentActiveAfter(prev_block, m_impl->m_opts.m_chainman, Consensus::DEPLOYMENT_SEGWIT));
}
...
void BlockDownloadManagerImpl::FindNextBlocksToDownload(NodeId nodeid, const BlockDownloadPeerInfo& info, unsigned int count,
                                                        std::vector<const CBlockIndex*>& vBlocks,
                                                        NodeId& nodeStaller)
{
    if (count == 0) return;
    ...
    FindNextBlocks(vBlocks, nodeid, *state, info, pindexWalk, count, nWindowEnd, &m_opts.m_chainman.ActiveChain(), &nodeStaller);
}

void BlockDownloadManagerImpl::TryDownloadingHistoricalBlocks(NodeId nodeid, const BlockDownloadPeerInfo& info, unsigned int count,
                                                              std::vector<const CBlockIndex*>& vBlocks,
                                                              const CBlockIndex* from_tip,
                                                              const CBlockIndex* target_block)
{
    ...
    FindNextBlocks(vBlocks, nodeid, *state, info, from_tip, count,
                                              ...
                                              NodeId* nodeStaller)
{
    ...
    bool is_limited_peer = info.m_is_limited_peer;
    NodeId waitingfor = -1;
